...
const liangcards = {
	...
	shfenghuo: {
		fullskin: true,
		image: "ext:水泊娘山/card/image/shfenghuo.png",
		derivation: "sh_lingzhen",
		type: "equip",
		subtype: "equip1",
		skills: ["shfenghuo_skill"],
		destroy: true,
		ai: {
			basic: {
				equipValue: 2,
	...
	shjinlun: {
		fullskin: true,
		image: "ext:水泊娘山/card/image/shjinlun.png",
		derivation: "sh_lingzhen",
		type: "equip",
		subtype: "equip1",
		distance: {
	...
	shzimu: {
		fullskin: true,
		derivation: "sh_lingzhen",
		image: "ext:水泊娘山/card/image/shzimu.png",
		type: "equip",
		subtype: "equip1",
		distance: {
			attackFrom: -4,
	...
	danshutiequan: {
		fullskin: true,
		derivation: "sh_chaijin",
		image: "ext:水泊娘山/card/image/danshutiequan.png",
		type: "equip",
		subtype: "equip5",
		skills: ["danshutiequan_skill", "danshutiequan_skill2"],
		ai: {
			equipValue: 9,
			basic: {
				order: (card, player) => {
	...
	shxuantianhunyuanjian: {
		fullskin: true,
		derivation: "sh_baodaoyi",
		image: "ext:水泊娘山/card/image/shxuantianhunyuanjian.png",
		type: "equip",
		subtype: "equip1",
		distance: { attackFrom: -6 },
		skills: ["shxuantianhunyuanjian_skill"],

		onEquip() {
			player.storage.shxuantianhunyuanjian_skill = false;
			player.markSkill("shxuantianhunyuanjian_skill");
		},
		onLose() {
			delete player.storage.shxuantianhunyuanjian_skill;
			player.unmarkSkill("shxuantianhunyuanjian_skill");
		},
	...
	shjinjiashenren: {
		fullskin: true,
		derivation: "sh_qiaodaoqing",
		image: "ext:水泊娘山/card/image/shjinjiashenren.png",
		type: "equip",
		subtype: "equip5",
		skills: ["shjinjiashenren_skill"],
		destroy: true,
		async onEquip(event, trigger, player) {
			var list = [];
			for (var name of lib.inpile) {
				if (get.subtype(name) == "equip1") list.add(name);
			}
			player.storage.shjinjiashenren = [];
			player.storage.shjinjiashenren[0] = player.group;
			player.storage.shjinjiashenren[1] = list[Math.floor(Math.random() * list.length)];
			game.log(player, "随机到了", get.translation(player.storage.shjinjiashenren[1]));
		...
		async onLose(event, trigger, player) {
			await player.changeGroup(player.storage.shjinjiashenren[0] == "shen" ? lib.character[player.name][1] : player.storage.shjinjiashenren[0]);
			player.removeAdditionalSkill("shjinjiashenren_skill");
			delete player.storage.shjinjiashenren;
...
const cardTranslates = {
	...
	danshutiequan: "丹书铁券",
	danshutiequan_info: "锁定技。①若你的手牌数不少于手牌上限，你的摸牌阶段改为与零至三名装备区有牌的其他角色各视为使用一张【树上开花】；反之你额外摸两张牌。②你的手牌上限等于一号位的体力上限。③你受到不小于你当前体力值的伤害时，你可弃置装备区的此牌，防止此伤害。",
};
