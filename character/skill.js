...
const skills = {
        ...
        mod: {
          aiOrder: function (player, card, num) {
            if (typeof card == "object" && player == _status.currentPhase) {
              ...
              if (
                numx < player.storage.shqinhui_draw &&
        ...
        async content(event, trigger, player) {
          ...
          if (player.storage.shqinhui_draw == numx) {
            game.playAudio(
              "..",
              "extension",
              "水泊娘山/audio/skill/shqinhui_draw.mp3",
            );
            await player.draw(3);
          }
  ...
  shxingbin: {
    ...
    content: function () {
      ...
      next.set("ai", function (button) {
        ...
        var val =
          card.name == "tao" || card.name == "jiu" ? 10 : get.value(card);
      ...
      if (event.recover.length) {
        ...
        var next = player.chooseBool(
          "是否让" + get.translation(event.recover) + "回复一点体力？",
        );
        next.set("goon", num > 0);
    ...
    ai: {
      ...
      order: function () {
        ...
        var x = Math.min(
          3,
          game.filterPlayer((current) => get.attitude(player, current) > 1)
            .length - 1,
        );
  ...
  shboshi: {
    ...
    filter: function (event, player) {
      ...
      return (
        (get.type(event.card) == "basic" || get.type2(event.card) == "trick") &&
        event.targets &&
    ...
    ai: {
      effect: {
        target_use: function (card, player, target, current) {
          ...
          if (
            (get.type(card) == "basic" || get.type2(card) == "trick") &&
            target != player &&
            !target.hasHistory("useSkill", (evt) => evt.skill == "shboshi")
          ) {
  ...
  shzhenwei: {
    ...
    async content(event, trigger, player) {
      ...
      let choiceList = [
        ...
        "令" + get.translation(player) + "移动你场上一张牌",
        "令" + get.translation(player) + "视为对你使用刺【杀】",
        "背水！对" + get.translation(player) + "造成1点伤害并执行所有选项",
      ];
      ...
      if (list.length == 3) list.push("背水！");
        ...
        choiceList[3] =
          '<span style="opacity:0.5; ">' + choiceList[3] + "</span>";
      ...
      const { control } = await target
        ...
        .set("ai", function () {
          ...
          if (get.attitude(target, player) > 1 && target.countCards("ej") > 0)
            return "选项二";
          if (target.countCards("j") && !target.countCards("e"))
            return "选项二";
          let bool1 = target.hasCard(
            (card) => get.value(card, target) < num * 5,
            "h",
          );
          let bool2 = target.hasCard(
            (card) => get.value(card, target) < num * 3,
            "e",
          );
          if (
            bool1 &&
            bool2 &&
            get.damageEffect(player, target, target) > 1 &&
            list.includes("背水！")
          )
            return "背水！";
      ...
      if (control) {
        ...
        if (control == "选项二" || control == "背水！") {
          player
            ...
            .set("prompt", "移动" + get.translation(target) + "场上一张牌");
        }
  ...
  shduogui: {
    ...
    filter: function (event, player) {
      return (
        player.countCards("h", { type: "equip" }) > 0 &&
        !player.hasSkill("shduogui_block")
      );
    },
    ...
    ai: {
      ...
      result: {
        ...
        target: function (player, target) {
          ...
          if (!card) return 0;
          if (get.attitude(player, target) > 2 && target.countCards("h") < 3)
            return 0;
    ...
    async content(event, trigger, player) {
      const result = await player
        .gainPlayerCard(
          trigger.player,
          "e",
          get.prompt("shdaozhui", trigger.player),
        )
  ...
  shyaomeng: {
    ...
    prompt: function (event, player) {
      ...
      return (
        ...
        "交给" +
        get.translation(event.player == Next ? Previous : Next)
      );
    },
    content: function () {
      "step 0";
  ...
  shdongzhu: {
    ...
    round: 1,
    check: function (event, player) {
      var cs = [ui.cardPile.childNodes[0], ui.cardPile.childNodes[1]].concat(
        player.getCards("h"),
      );
      ...
      if (
        ...
        !event.player.getCards("j").length
      )
        return false;
      for (var i of cs) {
        if (judge) {
          if (get.judge(judge)(i) * get.attitude(player, event.player) > 0)
            return true;
        } else break;
      }
    ...
    content: function () {
      ...
      next.set("filterMove", function (from, to, moved) {
        if ((to == 0 && moved[0].length > 2) || (from == 1 && to == 1))
          return false;
      ...
      next.processAI = function (list) {
        var check = function (card) {
          ...
          var num2 = player.hp,
            num1 = target.hp;
          var val =
            (num1 * get.value(card, player) + num2 * get.value(card, target)) /
            (num1 + num2);
          return val * att;
        };
        var player = _status.event.player;
  ...
  shzhanji: {
    ...
    content: function () {
      ...
      if (result.bool && result.cards) {
        ...
        if (X >= 4 && player.storage.shzhanji[0] <= 4)
          player.storage.shzhanji[0]++;
      } else {
        player.addTempSkill("shzhanji_2", "phaseUseEnd");
        ...
        mod: {
          selectTarget: function (card, player, range) {
            if (card.name == "sha" && range[1] != -1)
              range[1] += player.storage.shzhanji[2];
          },
  ...
  shlingwu: {
    ...
    check: function (event, player) {
      if (game.countPlayer((current) => player.inRange(current)) < 2)
        return false;
    ...
    content: function () {
      ...
      target
        .chooseCard(
          "he",
          "将一张牌交给" + get.translation(player) + "或令其弃置一张手牌",
        )
        .set("ai", function (card) {
        ...
        .set(
          "goon",
          (get.attitude(target, trigger.player) > 1 && event.num2 > 1) ||
            (get.attitude(target, player) > 2 && player.countCards("h") < 2),
        );
      ("step 2");
      if (result.bool) {
  ...
  shsixing2: {
    ...
    direct: true,
    filter: function (event, player) {
      if (event.player.countCards("he") <= 0 || event.player == player)
        return false;
    ...
    async content(event, trigger, player) {
      const result = await player
        .gainPlayerCard(
          ...
          "hej",
          [1, Infinity],
          get.prompt("shsixing", trigger.player),
        )
        .set("ai", function (button) {
          const card = button.link,
            player = get.event().player;
      ...
      if (result.bool) {
        game.playAudio(
          "..",
          "extension",
          "水泊娘山/audio/skill/shsixing21.mp3",
        );
        const num = result.cards.length;
        player
  ...
  shyuxun: {
    ...
    filter: function (event, player) {
      var moreMax = game.hasPlayer((current) => {
        return (
          ...
          current.countCards("hej") >= event.player.countCards("hej")
        );
      });
    ...
    content: function () {
      ...
      if (player.storage.shsixing && player.storage.shsixing < 2)
        player.storage.shsixing++;
      ("step 1");
        ...
        if (targets && targets.length > 0) {
          ...
          for (var current of targets) {
            var att = Math.min(
              ...
              get.attitude(current, player),
            );
            if (cards.length)
              gainvalue += att * (get.value(cards) / cards.length);
            if (
      ...
      if (result.bool) {
        if (
          player.hasHistory("useCard", (evt) => {
            return (
              evt.card.name == "jiejia" &&
              evt.getParent(2).name == "shanxiang" &&
  ...
  shpixian: {
    ...
    intro: {
      ...
      content: function (storage, player) {
        ...
        var num = (storage || 0) % 3;
        return (
          "转换技，出牌阶段，你可以和座次小于你的角色交换" +
          list[num] +
          "。若你以此法交换座次或交换牌差不小于3，辟险失效至你进入濒死状态。"
        );
      },
    },
    enable: "phaseUse",
    ...
    async content(event, trigger, player) {
      ...
      if (dif > 2)
        await player.addTempSkill("shpixian_block", { player: "dying" });
    },
    ai: {
      threaten: 2.4,
      order: 1,
        ...
        target: function (player, target) {
          ...
          if (list[num] == "手牌")
            value =
              ...
              get.value(target.getCards("h"), target);
          if (list[num] == "装备")
            value =
  ...
  shpengshuai: {
    ...
    filter: function (event, player) {
      if (!player.countCards("h", (card) => get.type2(card) == "trick"))
        return false;
      return (
    ...
    subSkill: {
      buff: {
        ...
        intro: {
          content:
            "下回合的摸牌阶段额外摸牌、出牌阶段额外出【杀】、弃牌阶段额外存牌",
        },
        content: function () {
        ...
        mod: {
          cardUsable: function (card, player, num) {
            if (card.name == "sha")
              return num + player.countMark("shpengshuai_buff");
          },
          maxHandcard: function (player, num) {
            if (
              _status.event.name == "phaseDiscard" ||
              _status.event.getParent("phaseDiscard").name == "phaseDiscard"
            )
              return num + player.countMark("shpengshuai_buff") * 3;
          },
        },
        sub: true,
      },
  ...
  shbaoshi: {
    ...
    content: function () {
      ...
      var next = player.chooseToDiscard(
        "he",
        "弃置一张牌并获得" + get.translation(trigger.cards),
        true,
      );
      next.set("ai", function (card) {
  ...
  shweishu: {
    ...
    filter: function (event, player) {
      ...
      return (
        ...
        player.countCards(
          "he",
          (card) => (get.suit(card) == get.suit(event.card)) > 0,
        )
      );
    },
    async content(event, trigger, player) {
        ...
        .set("ai", (card) => {
          ...
          if (list.length) {
            max =
              get.effect(
                target,
                { name: list[0][2], nature: list[0][3], isCard: true },
                trigger.player,
                player,
            ...
            for (var i of list) {
              if (
                get.effect(
                  target,
                  { name: i[2], nature: i[3], isCard: true },
                  trigger.player,
                  player,
                ) -
                  get.effect(target, trigger.card, trigger.player, player) >
      ...
      if (result?.cards?.length) {
        ...
        const resultx = await player
          .chooseButton(
            ["请声明一张即时牌，此后按你所声明的牌结算", [list, "vcard"]],
            true,
          )
          .set("ai", function (button) {
        ...
        if (resultx.bool) {
          ...
          if (trigger.card.name == "sha")
            trigger.card.nature = resultx.links[0][3];
    ...
    getCanChangeList: function (cardx, card) {
      ...
      for (var name of lib.inpile) {
        ...
        if (get.type(name) == "basic" || get.type(name) == "trick") {
          if (
            (get.name(cardx) == name && name != "sha") ||
            get.cardNameLength(cardy) != num
          )
            continue;
          list.push([get.translation(get.type(name)), "", name]);
          if (name == "sha") {
            for (var j of lib.inpile_nature) {
              if (j != get.nature(cardx) || get.name(cardx) != "sha")
                list.push(["基本", "", "sha", j]);
            }
          }
        }
      ...
      if (targets && targets.length == 2) {
        ...
        const result = await player
          ...
          .set("filterButton", function (button) {
            return (
              targets1.canEquip(button.link) && get.suit(button.link) == suit
            );
          })
          .forResult();
        if (result?.cards?.length) {
  ...
  shqiema: {
      ...
      return game.hasPlayer((current) => {
        return (
          current.countCards("e", (card) =>
            ["equip4", "equip3", "equip6"].includes(get.subtype(card)),
          ) > 0
        );
      });
    },
    ...
    content: function () {
      ...
      event.noSha = true;
      var max =
        targets[0].getSeatNum() > targets[1].getSeatNum()
          ? targets[0].getSeatNum()
          : targets[1].getSeatNum();
      var min =
        targets[0].getSeatNum() < targets[1].getSeatNum()
          ? targets[0].getSeatNum()
          : targets[1].getSeatNum();
        ...
        current
          ...
          .set("filterTarget", function (card, player, target) {
            ...
            if (
              target != Targets[0] &&
              target != Targets[1] &&
              !ui.selected.targets.includes(Targets[0]) &&
              !ui.selected.targets.includes(Targets[1])
            )
              return false;
      ...
      player
        ...
        .set("filterButton", function (button) {
          var targets1 = _status.event.targets1;
          return (
            targets1.canEquip(button.link) &&
            ["equip4", "equip3", "equip6"].includes(get.subtype(button.link))
          );
        });
      ("step 5");
      if (
    ...
    subSkill: {
      fqjz: {
        ...
        filter: function (event, player) {
          ...
          return game.hasPlayer(function (current) {
            return (
              current.hasSkill("shhuoqi") && current == _status.currentPhase
            );
          });
        },
        content: function () {
    ...
    content: function () {
      ...
      if (result.bool && result.cards && result.cards.length) {
        ...
        if (get.color(result.cards[0]) == "red")
          event.red.push(event.targets[event.num2]);
        if (get.type(result.cards[0]) != "equip")
          event.unequip.push(event.targets[event.num2]);
      }
      ("step 5");
      event.num2++;
  ...
  shleiju: {
    ...
    check: function (event, player) {
      ...
      if (
        ...
        player.countCards("h", (card) => player.hasValueTarget(card)) > 0
      )
        return false;
      return true;
    },
    content: function () {
      ...
      ("step 1");
      if (result.bool) {
        game.playAudio("..", "extension", "水泊娘山/audio/skill/shleiju1.mp3");
  ...
  shyafeng: {
    ...
    filter: function (event, player) {
      ...
      if (player.getStat("skill").shyafeng > 0)
        return (
          lib.skill.shmifeng.getSuit().includes("club") &&
          lib.skill.shmifeng.getSuit().length == 1
        );
      return true;
    },
    viewAs: {
    ...
    mod: {
      aiOrder: function (player, card, num) {
        if (typeof card == "object" && player == _status.currentPhase) {
          if (get.name(card) == "shunshou" && get.suit(card, false) == "club")
            return num + 10;
        }
      },
    },
    ...
    subSkill: {
      buff: {
        ...
        content: function () {
          ...
          if (
            !player.storage.shhuoshan_blocker ||
            player.storage.shhuoshan_blocker == 0
          )
            player.setStorage("shhuoshan_blocker", 1);
  ...
  shquexi: {
    ...
    content: function () {
      ...
      var next = player
        .chooseControl(list)
        .set("prompt", "摸一张牌或重铸所有牌，然后回复1点体力");
      next.set("ai", function (button) {
        var player = _status.event.player;
    ...
    ai: {
      ...
      result: {
        player: function (player) {
          if (player.hasValueTarget({ name: "sha", nature: "thunder" }, false))
            return 1;
          return 0;
        },
      },
      ...
      do {
        const { bool, links } =
          cards.length == 1
            ...
            : await player
                .chooseCardButton("请选择要分配的牌", true, cards, [
                  1,
                  cards.length,
                ])
                .set("ai", () => {
  ...
  shjiayao: {
    ...
    subSkill: {
      global: {
        ...
        filter: function (event, player) {
          var card = event.card;
          if (get.name(card) != "jiu" || get.color(card) == "none")
            return false;
  ...
  shpingyue: {
    ...
    filter: function (event, player) {
      if (
        player.hasSkill("shpingyue_fire") &&
        player.hasSkill("shpingyue_thunder") &&
        player.hasSkill("shpingyue_ice")
      )
        return false;
      return player.countCards("h") > 0;
    },
    async content(event, trigger, player) {
      ...
      for (var i of natures) {
        if (!player.hasSkill("shpingyue_" + i))
          list2.push(["基本", "", "sha", i]);
      }
      if (list2.length > 0) {
        togiver.sortBySeat();
        const { bool, links } = await player
          .chooseButton([
            "视为对" + get.translation(togiver) + "使用一张【杀】？",
            [list2, "vcard"],
          ])
          .set("ai", function (button) {
          ...
          .forResult();
        if (bool) {
          await player.addTempSkill(
            "shpingyue_" + links[0][3],
            "phaseUseAfter",
          );
    ...
    ai: {
      ...
      result: {
        player: function (player, target) {
          ...
          var num = player
            .getCards("h")
            .reduce((sum, card) => sum + get.value(card), 0);
          var bool2 =
  ...
  shxuanzhen: {
        ...
        return (
          current.countCards(
            ...
            (card) =>
              ...
              !player.storage.shxuanzhen.includes(card),
          ) > 0
        );
      });
    },
    ...
    content: function () {
      ...
      event.num1 = game.filterPlayer((current) =>
      ...
      player
        ...
        .set("filterButton", function (button) {
          ...
          return (
            targets1.canEquip(button.link) &&
            (!player.storage.shxuanzhen ||
              !player.storage.shxuanzhen.includes(button.link))
          );
        });
      ("step 2");
      ...
      if (num2 > event.num1) {
        game.playAudio(
          "..",
          "extension",
          "水泊娘山/audio/skill/shxuanzhen1.mp3",
        );
        ...
        game.playAudio(
          "..",
          "extension",
          "水泊娘山/audio/skill/shxuanzhen2.mp3",
        );
        player.draw(2);
      ...
      result: {
        target: function (player, target) {
          var players = game.filterPlayer((current) => {
            return (
              current.countCards("e") > 0 && get.attitude(player, current) < 0
            );
          });
        ...
        content: function () {
          ...
          if (event.choice == "横置") {
            game.playAudio(
              "..",
              "extension",
              "水泊娘山/audio/skill/shlingchen1.mp3",
            );
            player.link(true);
          ...
          if (event.choice == "重置") {
            game.playAudio(
              "..",
              "extension",
              "水泊娘山/audio/skill/shlingchen2.mp3",
            );
            player.link();
  ...
  shzhenjiu: {
    ...
    filter: function (event, player) {
      if (
        event.player == player ||
        event.player.isDead() ||
        player.hasSkill("shzhenjiu_round")
      )
        return false;
      return player.countCards("he") > 0;
    },
      ...
      player
        ...
        .set("ai", function (card) {
          ...
          var num = target.countCards(
            "h",
            (card) => get.name(card, target) == "jiu",
          );
          var num1 = target.countCards(
            "h",
            (card) =>
              card.hasGaintag("visible_shjiepeng") &&
              get.color(card) == "black",
          );
          var att = get.attitude(player, target);
      ...
      exchange: {
        ...
        content: function () {
          game.playAudio(
            "..",
            "extension",
            "水泊娘山/audio/skill/shzhenjiu_exchange1.mp3",
          );
        ...
        content: function () {
          ...
          player
            .chooseTarget(
              "令【五谷丰登】对至多" +
                get.cnNumber(
                  Math.min(trigger.targets.length, trigger.cards.length),
                ) +
                "名目标无效",
              [1, Math.min(trigger.targets.length, trigger.cards.length)],
    ...
    ai: {
      ...
      result: {
        target: function (player, target) {
          ...
          return (1 -
            get.distance(sorter, target, "absolute") / game.countPlayer()) *
  ...
  shcangren: {
    ...
    filter: function (event, player) {
      ...
      if (get.type(cards[0]) == "delay" || get.type(cards[0]) == "equip")
        return false;
    ...
    check: function (event, player) {
      var cards = event.getg(event.player);
      var name = get.name(cards[0], event.player);
      if (get.effect(event.player, { name: name }, player, player) > 0)
        return true;
      return false;
    },
    prompt: function (event, player) {
      ...
      return (
        ...
        "使用一张" +
        get.translation(get.name(cards[0], event.player)) +
        "？"
      );
    },
    content: function () {
    ...
    content: function () {
      ...
      if (!cards.length) event.finish();
        ...
        var next = player
          .chooseButton(
            [
              get.prompt("shzhijing"),
              "令" + get.translation(trigger.player) + "获得任意张花色不同的牌",
              cards,
            ],
            [1, 4],
          )
        ...
        next.set("filterButton", function (button) {
          if (ui.selected.buttons.length) {
            for (var i of ui.selected.buttons) {
              if (get.suit(button.link, false) == get.suit(i, false))
                return false;
            }
          }
          return true;
      ...
      if (result.bool) {
        ...
        if (!trigger.player.storage.shzhijing_buff)
          trigger.player.storage.shzhijing_buff = [];
      ...
      buff: {
        ...
        mod: {
          cardEnabled: function (card, player) {
            if (player.getStorage("shzhijing_buff").includes(get.suit(card)))
              return false;
          },
    ...
    subSkill: {
      viewas: {
        ...
        filter: function (event, player) {
          ...
          return (
            player.hasCard((card) => player.canRecast(card), "he") > 0 &&
            player.hp == 1
          );
        },
        ...
        content: function () {
          ...
          player
            .chooseCard(
              "he",
              get.prompt2("shmifeng"),
              lib.filter.cardRecastable,
              true,
            )
            .set("ai", function (card) {
              if (!_status.event.suits.includes(get.suit(card)))
                return 100 - get.value(card);
              if (
                get.name(card) == "shan" ||
                get.name(card) == "tao" ||
      ...
      const { bool, cards, targets } = await player
        .chooseCardTarget({
          ...
          filterCard(card, player) {
            ...
            return (
              target1.countCards("ej", { suit: suit }) > 0 &&
          ...
          filterTarget(card, player, target) {
            ...
            for (var i of es) {
              if (get.position(i) == "j") {
                if (target.canAddJudge(i) && !target2s.includes(target))
                  return true;
              }
      ...
      const result = await player
        ...
        .set("filterButton", function (button) {
          if (get.position(button.link) == "j")
            return (
              target2.canAddJudge(button.link) && get.suit(button.link) == suit
            );
  ...
  shguahuo: {
    ...
    mod: {
      targetEnabled: function (card, player, target, now) {
        if (
          card.name == "tao" &&
          !player.hasSkill("shguahuo_alloc") &&
          target.isDying()
        )
          return false;
      },
  ...
  shjianghuo: {
    ...
    precontent() {
      ...
      player
        ...
        .filter(
          (evt) =>
            evt.skill == "shjianghuo" &&
            evt.card.storage &&
            evt.card.storage.shjianghuo,
        )
  ...
  shbingyin: {
    ...
    filter: function (event, player) {
      ...
      return game.hasPlayer(
        (current) => !current.isLinked() && current.countCards("e") > 0,
      );
    },
    direct: true,
    content: function () {
  ...
  shjinbing: {
    ...
    content: function () {
      ...
      player
        .chooseTarget(
          get.prompt("shjinbing"),
          "是否视为对一名装备区牌数最多的角色使用一张【水淹七军】？",
          (card, player, target) => {
            return (
              target.isMaxEquip() &&
              player.canUse({ name: "shuiyanqijunx", isCard: true }, target)
            );
          },
        )
        .set("ai", (target) => {
      ...
      damage: {
        ...
        filter: function (event, player) {
          if (
            !event.card ||
            !event.card.storage ||
            !event.card.storage.shjinbing
          )
            return false;
          return (
            event.card.name == "shuiyanqijunx" &&
            event.player != player &&
            event.player.isIn()
          );
        },
        ...
        if (!hs.length || numx < 1) event.finish();
          ...
          if (numx > 0)
            player.chooseCard(
              ...
              true,
              numx,
              "交给" + get.translation(target) + prompt,
            );
        }
      } else event.finish();
      ("step 2");
  ...
  shjinlun_skill: {
    ...
    filter: function (event, player) {
      if (
        player.countCards(
          "h",
          (card) => get.color(card) == "red" && get.type(card) == "basic",
        )
      ) {
        return true;
      }
    ...
    subSkill: {
      choose: {
        ...
        filter: function (event) {
          var evt = event;
          return (
            evt.card.name == "sha" && evt.skill == "shjinlun_skill" && evt.cards
          );
        },
        content: function () {
          "step 0";
    ...
    ai: {
      ...
      skillTagFilter(player, tag, arg) {
        if (
          (arg &&
            arg.name == "sha" &&
            arg.target.getEquip(2) &&
            arg.target.isLinked() &&
            get.color(arg.card) == "red") ||
          (!arg.target.isLinked() && get.color(arg.card) == "black")
        )
          return true;
        return false;
      },
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        for (const target of targets)
          await target.chooseUseTarget("kaihua", true);
      }
      trigger.changeToZero();
      await game.delay();
    },
  ...
  shjinjiashenren_skill: {
    init: function (player) {
      ...
      if (player.storage.shjinjiashenren) {
        player.addAdditionalSkill("shjinjiashenren_skill", [
          player.storage.shjinjiashenren[1] + "_skill",
        ]);
      }
    },
    ...
    ai: {
      ...
      skillTagFilter: function (player, tag, arg) {
        if (
          ...
          arg.card &&
          arg.card.name == "sha" &&
          player.hp < player.getAttackRange()
        )
          return true;
        return false;
      },
  ...
  shlianzheng: {
    ...
    intro: {
      content: function (storage, player, skill) {
        var str = player.storage.shlianzheng
          ? "出牌阶段限一次，你可以将X张手牌当【决斗】使用，然后于你因之造成伤害后摸两张牌（X为你本回合造成过的伤害值+1）。"
          : "出牌阶段限一次，你可以视为使用一张【决斗】，然后于之结算完成后失去1点体力。";
        return str;
      },
    },
    enable: "phaseUse",
    ...
    prompt: function () {
      ...
      var num = player.getStat().damage || 0;
      if (player.storage.shlianzheng == true)
        return "将" + get.cnNumber(num + 1) + "张牌当【决斗】使用";
      return "视为使用一张【决斗】";
    },
    check(card) {
    ...
    subSkill: {
      1: {
        ...
        filter(event, player) {
          ...
          const opposite =
            event.player === player ? event.target : event.player;
      ...
      lose: {
        ...
        filter: function (event, player) {
          ...
          return (
            event.card.storage &&
            event.card.storage.shlianzheng_lose &&
            event.card.storage.shlianzheng_lose == true
          );
        },
        content: function () {
          player.loseHp();
      ...
      draw: {
        ...
        filter: function (event, player) {
          ...
          return (
            event.card.storage &&
            event.card.storage.shlianzheng_draw &&
            event.card.storage.shlianzheng_draw == true &&
            event.player != player
          );
        },
        content: function () {
    ...
    ai: {
      ...
      result: {
        ...
        player: function (player, target, card) {
          ...
          if (
            hs1.length > hs2.length &&
            (!hs2.length || hs1[0].number > hs2[0].number)
          ) {
            return -2;
          }
          return -0.5;
  ...
  shhuixiong: {
    ...
    async content(event, trigger, player) {
      ...
      if (resultx?.bool && resultx.targets?.length) {
        ...
        if (
          target1.countCards("h") <= player.countCards("h") &&
          target2.countCards("h") <= player.countCards("h")
        ) {
          const result = await player
            .chooseTarget(
              "对其中一名目标角色造成1点伤害",
    ...
    ai: {
      ...
      result: {
        ...
        target: (player, target, card) => {
          ...
          let sha = game.filterPlayer(
              get.info({ name: "jiedao" }).filterAddedTarget,
            ),
            addTar = null;
  ...
  shhenglue: {
    ...
    selectTarget: function () {
      ...
      if (!ui.selected.targets || !ui.selected.targets.includes(player))
        return ui.selected.targets.length + 2;
      return [1, 3];
    },
    filterTarget: function (card, player, target) {
      if (player.isLinked())
    ...
    prompt: function () {
      var player = _status.event.player;
      if (player.isLinked())
        return "将一张红色手牌当无距离限制但仅可选择场上牌的【顺手牵羊】使用";
    ...
    onuse: function (links, player) {
      if (player.isLinked())
        game.playAudio(
          "..",
          "extension",
          "水泊娘山/audio/skill/shhenglue1.mp3",
        );
      else
        game.playAudio(
          "..",
          "extension",
          "水泊娘山/audio/skill/shhenglue2.mp3",
        );
    },
        ...
        target: function (card, player, target) {
          if (card.name == "tiesuo" && target.hasSkill("shjinchong")) {
            ...
            if (!target.isLinked() && !target.isTurnedOver())
              return [
                -0.5,
                3 * get.effect(target, { name: "draw" }, player, player),
              ];
    ...
    subSkill: {
      global: {
        ...
        filterTarget(card, player, target) {
          ...
          return (
            !target.hasSkill("shpiaopeng_round") &&
            target.hasSkill("shpiaopeng")
          );
        },
        ...
        async content(event, trigger, player) {
          game.playAudio(
            "..",
            "extension",
            "水泊娘山/audio/skill/shpiaopeng1.mp3",
          );
          const target = event.target;
  ...
  shqiongji: {
    ...
    async content(event, trigger, player) {
      const dialog = [],
        target = event.target;
      dialog.push(
        "穷计：弃置你与" + get.translation(player) + "的共计两张手牌",
      );
      ...
      if (bool) {
        ...
        const name =
          get.color(links[0], false) == get.color(links[1], false)
            ? "wuzhong"
            : "guohe";
    ...
    subSkill: {
      effect: {
        ...
        filter: function (event, player) {
          ...
          return (
            evt.card.storage &&
            evt.card.storage.shqiongji &&
            evt.card.storage.shqiongji > 0
          );
        },
        content: function () {
          "step 0";
      ...
      do {
        const { bool, links } =
          cards.length == 1
            ? { links: cards.slice(0), bool: true }
            : await player
                .chooseCardButton("狡智：请选择要分配的牌", true, cards, [
                  1,
                  cards.length,
                ])
                .set("ai", () => {
    ...
    ai: {
      ...
      effect: {
        target(card, player, target, current) {
          ...
          if (target.countCards("h") == 0 && list.includes(card.name))
  ...
  shguanshi: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("ai", (target) => {
          return (
            target.countCards("h") - get.attitude(get.event().player, target)
          );
        })
        .forResult();
      if (result.bool) {
      ...
      const { bool, links } = await player
        ...
        .set("ai", function (button) {
          if (get.attitude(get.event().player, get.event().target) < 1)
            return 2;
          return 0;
        })
        .set("target", target1x)
      ...
      if (cards2.length == 1) {
        ...
        const result = await player
          .chooseTarget(
            "是否将" + get.translation(cards2) + "交给一名角色",
            function (card, player, target) {
  ...
  shkunbian: {
    ...
    content: function () {
      ...
      player
        ...
        .then(() => {
          // game.log(1)
          var cards = trigger.target.getCards(
            "h",
            (card) =>
              card.hasGaintag("shkunbian") &&
              lib.filter.cardDiscardable(card, trigger.target, "shkunbian"),
          );
  ...
  shtujie: {
    ...
    async content(event, trigger, player) {
      const result = await player
        .discardPlayerCard(
          trigger.target,
          get.prompt("shtujie", trigger.target) +
            "弃置" +
        ...
        player
          ...
          .then(() => {
            if (trigger.player.isIn())
              trigger.player.draw(trigger.card.storage[0]);
  ...
  shlianji: {
    ...
    async content(event, trigger, player) {
      ...
      if (result1.bool) {
        ...
        log = true;
        for (const target of result1.targets.sortBySeat())
      ...
      if (
        ...
        !game.hasPlayer((cur) => cur.countCards("e") > 0)
      )
        return;
        ...
        game.hasPlayer((current) => {
          return (
            current.countCards("e", (card) => {
              return (
                ["equip4", "equip3", "equip6"].includes(get.subtype(card)) &&
                !player.storage?.shlianji.includes(card)
              );
            }) > 0
          );
        })
  ...
  shyubing: {
    ...
    async cost(event, trigger, player) {
      ...
      let choiceList = [
        "令任意名处于连环状态的角色摸一张牌",
        "重置" + get.translation(trigger.player) + "的武将牌并防止此伤害",
        "摸三张牌，然后重置场上所有角色武将牌",
      ];
      const result = await player
        .chooseControl(list)
        ...
        .set("ai", () => {
          ...
          if (
            ...
            target.hp + target.hujia <= get.event().num &&
            !player.countCards("h", { name: "tao" })
          )
            return list[1];
          if (get.event().targets.length > 2 && !get.event().hasNature)
            return list[0];
          return list[2];
        })
        ...
        .set(
          "targets",
          game.filterPlayer(
            (cur) => cur.isLinked() && get.attitude(player, cur) > 1,
          ),
        )
        .forResult();
      event.result = {
  ...
  shjuyi: {
          ...
          const result = await current
            ...
            .set("ai", function () {
              ...
              const num2 = Math.max(
                ...
                get.recoverEffect(evt.player, evt.player, get.event().player),
              );
              if (get.attitude(get.event().player, evt.player) < 1)
                return false;
              if (get.event().player.group != "liang")
                return get.attitude(get.event().player, evt.player) > 0;
              return num2 - num1 > 0;
            })
            .forResult();
          if (result.bool) {
    ...
    subSkill: {
      achieve: {
        ...
        filter: function (event, player) {
          const players = player.storage.shjuyi.filter((current) =>
            game.filterPlayer().includes(current),
          );
  ...
  shtiankui: {
    ...
    subSkill: {
      liang: {
        ...
        filter(event, player) {
          return (
            event.player.group == "liang" &&
            player.storage.shtiankui.includes(event.player)
          );
        },
        forced: true,
        async content(event, trigger, player) {
  ...
  shpozhen: {
    ...
    check: function (card) {
      ...
      if (ss.length) {
        let list = game
          ...
          .sort(function (a, b) {
            return (
              ...
              get.effect(a, { name: "sha", nature: "thunder" }, player, player)
            );
          });
        ...
        if (suits.length == 2 || suits.includes(get.suit(card, false)))
          return 0;
        return 5 - get.value(card);
      }
          ...
          if (
            targets.some((target) => {
              return (
                ...
                get.damageEffect(target, player, viewer, get.natureList(card)) >
                  0
              );
            })
          )
        ...
        if (typeof item === "object" && game.hasNature(item, "linked")) {
          if (
            ...
            game.countPlayer(function (current) {
              return (
                current.isLinked() &&
                get.damageEffect(current, player, player, get.nature(item)) > 0
              );
            }) > 1
          )
            return 3.1;
  ...
  shchouni: {
    ...
    animationColor: "thunder",
    check(event, player) {
      if (player.countCards("h", { name: "tao" }) > 0 || player.hp > 2)
        return false;
      return true;
    },
    async content(event, trigger, player) {
      ...
      if (get.mode() == "doudizhu" && player.identity != "fan")
        await player.addSkills(["feiyang", "bahu"]);
      else await player.addSkills(["jsrgfeiyang", "jsrgbahu"]);
  ...
  shwushuang: {
    ...
    async content(event, trigger, player) {
      ...
      await player
        .chooseUseTarget(
          card,
          get.prompt("shwushuang"),
          "视为使用一张需" + get.cnNumber(num) + "张【闪】才能抵消的冰【杀】",
          false,
          "nodistance",
        )
  ...
  shchanjin: {
    ...
    hiddenCard: (player, name) => {
      return (
        name == "tiesuo" &&
        player.hasCard(
          (card) => get.color(card) == "red" || get.type2(card) == "trick",
          "she",
        )
      );
    },
    ...
    check: function (card) {
      if (get.color(card) == "red" && get.type2(card) == "trick")
        return 7 - get.value(card);
      return 5 - get.value(card);
    },
    precontent() {
      ...
      if (
        cards.length &&
        cards.length == 1 &&
        get.color(cards[0]) == "red" &&
        get.type2(cards[0]) == "trick"
      ) {
        const suit = get.suit(cards[0], false);
        player
          .when("useCardToTargeted")
      ...
      islink: {
        ...
        mod: {
          cardEnabled2: function (card, player) {
            if (
              get.suit(card) == player.storage.shchanjin_link &&
              get.position(card) == "h"
            )
              return false;
          },
        ...
        intro: {
          content: function (storage, player) {
            return (
              "不能使用或打出" +
              get.translation(player.storage.shchanjin_link) +
              "手牌"
            );
          },
        },
  ...
  shbenjiu: {
    ...
    async content(event, trigger, player) {
      ...
      const list = [];
      let choiceList = [
        "将所有手牌交给" + get.translation(target) + ",然后其回复1点体力",
        "将" +
          get.translation(target) +
          "受到的伤害转移给你，然后与其各摸一张牌",
        "背水！此后对" + get.translation(target) + "强制发动奔救并执行所有选项",
      ];
      ...
      const { control } = await player
        ...
        .set(
          "prompt",
          player.storage.shbenjiu.includes(target)
            ? "请选择一项"
            : get.prompt("shbenjiu", target),
        )
        .set("ai", function () {
          const player = get.event().player;
          let att = get.attitude(player, target);
          let bool =
            ["君", "主"].includes(lib.translate[target.identity]) &&
            !["野", "内"].includes(lib.translate[player.identity]);
          let bool1 =
            (get.value(player.getCards("h"), player) <
              get.value(player.getCards("h"), target) &&
              get.recoverEffect(target, player, player) > 1) ||
            bool;
          let bool2 = player.hp + player.hujia - trigger.num > 0 || bool;
          let unbool2 =
            get.damageEffect(target, trigger.source, player) > 0 ||
            (get.attitude(player, target) > 0 &&
              get.damageEffect(target, trigger.source, target) > 0);
  ...
  shyebing: {
    ...
    async content(event, trigger, player) {
      ...
      const { bool, links } = await player
        .chooseButton(
          [
            "令" + get.translation(target) + "获得“梁山兵甲库中的一件装备”",
            list,
          ],
          true,
        )
    ...
    ai: {
      order: function () {
        ...
        var hs = player.getCards("h");
        if (
          hs.some((card) => get.name(card) == "baiyin") &&
  ...
  shbazhen: {
    ...
    filter: function (event, player) {
      if (
        !event.cards ||
        !event.cards.some((card) => get.name(card, false) == "sha")
      )
        return false;
  ...
  shtongmai: {
    ...
    filterCard: function (card, player, event) {
      ...
      if (
        name == "tao" &&
        filter({ name: "sha", nature: "stab", cards: [card] }, player, event)
      )
        return true;
      return false;
    },
    filter: function (event, player) {
      ...
      if (
        filter(
          get.autoViewAs({ name: "sha", nature: "stab" }, "unsure"),
          player,
          event,
        ) &&
        ...
        info.onLose = function () {
          if (card.storage && card.storage.shaiai) {
            ...
            if (player.storage && player.storage.shaiai2) {
              player.storage.shaiai2 = player.storage.shaiai2.filter(
                function (elem) {
            ...
            card2.init([
              card.storage.shaiai[0],
              card.storage.shaiai[1],
              card.storage.shaiai[2],
            ]);
  ...
  shhanhan: {
    ...
    async content(event, trigger, player) {
      const { bool, links } = await trigger.source
        .chooseCardButton(
          "获得一张除外牌",
          player.getExpansions("shaiai"),
          true,
        )
        .forResult();
      if (!bool) return;
    ...
    filter: function (event, player) {
      return game.hasPlayer((current) => {
        return (
          ...
          !current.hasSkill("shkangge_blocker")
        );
      });
    },
    async content(event, trigger, player) {
      ...
      const result = await target
        .chooseToUse({
          filterCard: function (card, player, event) {
            if (get.suit(card, false) != suit || get.itemtype(card) != "card")
              return false;
            return lib.filter.filterCard.apply(this, arguments);
          },
  ...
  shshenxing: {
    ...
    async content(event, trigger, player) {
      ...
      for (var i in event.given_map) {
        var source = (_status.connectMode ? lib.playerOL : game.playerMap)[i];
        await source
          .addToExpansion(event.given_map[i], "gain2")
          .gaintag.add("shshenxing");
      }
    },
    onremove: function (player, skill) {
      var cards = player.getExpansions(skill);
  ...
  shzoutan: {
    ...
    filter: function (event, player) {
      ...
      if (event.player == player || get.distance(player, event.player) != 1)
        return false;
      ...
      const { bool, links } = await player
        ...
        .set("ai", function (button) {
          ...
          if (get.attitude(player, target) > 1)
            return (
              get.value(button.link, player) - get.value(button.link, target)
            );
          return get.value(button.link);
        })
        .forResult();
  ...
  shduofan: {
    ...
    async content(event, trigger, player) {
      ...
      const targets = (
        await player
          .chooseTarget(
            [1, Pro_targets.length],
            get.prompt2("shduofan"),
            function (card, player, target) {
      ...
      if (targets && targets.length) {
        ...
        if (targets.length == Pro_targets.length) {
          if (typeof player.storage.shduofan_range !== "number")
            player.storage.shduofan_range = 0;
          await player.addSkill("shduofan_range");
          player.storage.shduofan_range += 1;
          if (player.storage.shduofan_range != 0)
            player.markSkill("shduofan_range");
  ...
  shgengjue1: {
    ...
    filter: function (event, player) {
      if (
        event.type == "wuxie" ||
        !lib.skill.shgengjue1.getBasicList1(event, player).length
      )
        return false;
    ...
    getBasicList1: function (event, player) {
      ...
      for (let name of lib.inpile) {
        if (
          get.type(name) != "basic" ||
          history.some((evt) => evt.card.name == name)
        )
          continue;
        ...
        if (name == "sha") {
          for (let nature of lib.inpile_nature) {
            ...
            if (
              ...
              !history.some(
                (evt) =>
                  evt.card.name == "sha" && get.nature(evt.card) == nature,
              )
            )
  ...
  shgengjue2: {
    ...
    filter: function (event, player) {
      if (
        event.type == "wuxie" ||
        !lib.skill.shgengjue2.getBasicList2(event, player).length
      )
        return false;
      ...
      for (let name of lib.inpile) {
        ...
        var inDiscardPile = function (name, nature) {
          for (var i = 0; i < ui.discardPile.childElementCount; i++) {
            if (
              name == "sha" &&
              ui.discardPile.childNodes[i].name == name &&
              get.nature(ui.discardPile.childNodes[i], false) == nature
            )
              return true;
        ...
        let card = { name: name, isCard: true };
        if (event.filterCard(card, player, event) && inDiscardPile(name, null))
          vcards.push(["基本", "", name]);
        if (name == "sha") {
          for (let nature of lib.inpile_nature) {
            card.nature = nature;
  ...
  shgengjue3: {
    ...
    filter: function (event, player) {
      if (
        !player.countCards("hes", { type: "basic" }) ||
        !player.storage ||
        player.hasSkill("shgengjue3_blocker")
      )
        return false;
      return (
        lib.skill.shgengjue3.getBasicList3(event, player).length &&
        game.roundNumber >= 3
      );
    },
    chooseButton: {
          ...
          precontent: function () {
            ...
            player
              ...
              .then(() => {
                var num = 0;
                if (
                  player.storage.shgengjue1 &&
                  player.storage.shgengjue1.length &&
                  player.storage.shgengjue1.some(
                    (card) => card.name == cardx.name,
                  )
                ) {
                ...
                if (
                  player.storage.shgengjue2 &&
                  player.storage.shgengjue2.length &&
                  player.storage.shgengjue2.some(
                    (card) => card.name == cardx.name,
                  )
                ) {
    ...
    getBasicList3: function (event, player) {
      ...
      for (let name of lib.inpile) {
        ...
        if (
          !player.storage.shgengjue1.some((card) => get.name(card) == name) &&
          !player.storage.shgengjue2.some((card) => get.name(card) == name)
        )
          continue;
        if (name == "sha") {
          if (
      ...
      if (event.triggername == "phaseUseBegin") {
        ...
        const { bool, cards, targets } = await player
          .chooseCardTarget({
            ...
            prompt2:
              "将任意张牌交给一名其他角色，若交出牌不少于2，你不转换〖斡旋〗。",
            filterCard: true,
            selectCard: [1, Infinity],
            position: "he",
            ...
            ai1: function (card) {
              if (
                ...
                get.event().player.getFriends().length > 2
              )
                return 12 - get.value(card);
              return 5 - get.value(card);
            },
        ...
        const cresult = await current
          .chooseCard(
            ...
            "交给" +
              get.translation(player) +
              "任意张牌，若交出牌数不多于2，其不转换〖斡旋〗。",
          )
          .set("ai", (card) => {
            if (get.attitude(current, player) <= 0) return 0;
            else if (get.suit(card, player) == suit)
              return 12 - get.value(card);
            return 5 - get.value(card);
          })
          .forResult();
    ...
    intro: {
      content(storage, player, skill) {
        if (player.storage.shwoxuan == true)
          return "出牌阶段开始时，你可以将任意张牌交给一名其他角色，令一名角色回复1点体力并复原武将牌，若不少于两张，不转换〖斡旋〗。";
  ...
  shjijiu: {
    mod: {
      aiValue(player, card, num) {
        if (num <= 0 || get.itemtype(card) !== "card") return;
        if (
          !player.getExpansions("shbinhua").length ||
          player.getExpansions("shbinhua").length != 1
        )
          return;
        ...
        const cards = player.getCards(
          "hs",
          (card) => get.name(card) == "tao" || get.suit(card) != suit,
        );
        cards.sort(
          (a, b) =>
    ...
    viewAsFilter(player) {
      ...
      return (
        player != _status.currentPhase &&
        player.countCards("hes", { suit: suit }) > 0
      );
    },
    filterCard(card, player) {
      const suit = get.suit(player.getExpansions("shbinhua")[0], false);
    ...
    prompt: function () {
      const suit = get.suit(
        _status.event.player.getExpansions("shbinhua")[0],
        false,
      );
  ...
  shxingshao: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("ai", (target) => {
          if (target == target1 && player.hp > 1)
            return (
              2 * target.countCards("he", (card) => get.value(card, player) > 5)
            );
          return get.damageEffect(
            target,
            _status.event.player,
      ...
      if (result.bool) {
        ...
        if (chooselist.length == 1) {
          ...
          const { bool, links } = await target
            .chooseButton(
              [
                "鲸魂：选择一张武将牌置为" + get.translation(player) + "的副将",
                [chooselist, "character"],
              ],
              true,
            )
            .set("ai", (button) => {
    ...
    subSkill: {
      ...
      clear: {
        ...
        filter: function (event, player) {
          if (
            player.countCards("h", (card) =>
              card.hasGaintag("visible_shjinghun"),
            )
          )
            return false;
  ...
  shxuanfeng: {
    ...
    async cost(event, trigger, player) {
      const chooseList = lib.skill.shxuanfeng.getChooseMaxValueList(
        trigger,
        player,
      );
      const list = [];
      ...
      const result = await player
        ...
        .set("ai", () => {
          if (
            ...
            return "移动牌";
          if (list.includes("弃牌") && chooseList[0][0] == "弃牌")
            return "弃牌";
        ...
        do {
          ...
          if (result.bool) {
            ...
            const result2 = await player
              .discardPlayerCard([1, count], result.targets[0], "he", true)
              .forResult();
    ...
    getChooseMaxValueList: function (event, player) {
      ...
      const isMinLosePlayer = function (
        event,
        target,
        loseList00,
        loseList01,
      ...
      const getChooseMaxList = function (event, player, chooseBool) {
        if (chooseBool[0] == false) {
          ...
          if (chooseBool[1] == false) {
            ...
            var MaxValue = -5;
            for (const target of game.filterPlayer((current) =>
              player.canMoveCard(null, true, [current], game.filterPlayer()),
            )) {
              var Value = 0;
              ...
              for (var current of game.filterPlayer()) {
                if (
                  ...
                  if (get.attitude(player, current) < 0)
                    Value += Math.abs(
                      2 * get.damageEffect(current, player, current),
                    );
                  else
                    Value -= get.attitude(player, current) * (5 - current.hp);
                }
              }
              if (Value > MaxValue) {
                MaxValue = Value;
            ...
            for (const target1 of game.filterPlayer((current) =>
              ...
              if (get.attitude(player, target1) < 0)
                Value += Math.abs(
                  get.effect(target1, { name: "guohe_copy2" }, player, player),
                );
              else {
                  ...
                  for (var current of game.filterPlayer()) {
                    if (
                      ...
                      if (current == player) Value -= 9 * (5 - player.hp);
                        ...
                        Value -=
                          get.attitude(player, current) * (5 - current.hp);
                    }
                  }
                  if (Value > MaxValue) {
                    MaxValue = Value;
      ...
      draw: {
        ...
        filter: function (event, player) {
          return (
            event.card &&
            event.card.storage &&
            event.card.storage.shzhebang &&
            event.card.storage.shzhebang > 0 &&
            player.isIn() &&
  ...
  shtouchui: {
    ...
    filter: function (event, player) {
      return (
        get.tag(event.card, "damage") && player.countCards("h", { name: "sha" })
      );
    },
    async cost(event, trigger, player) {
      event.result = await player
        .chooseToDiscard(
          get.prompt("shtouchui", trigger.player),
          "弃置一张杀并对" + get.translation(trigger.player) + "造成1点伤害",
          { name: "sha" },
        )
        .set("ai", function (card) {
  ...
  shfeixing: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          get.prompt("shfeixing"),
          "令攻击范围内一名与你距离大于1的角色打出一张" +
            get.translation(get.suit(trigger.card)) +
            "杀或闪，否则受到你造成的1点伤害。",
          (card, player, target) => {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        .chooseToRespond((card, player) => {
          return (
            get.suit(card) == suit &&
            (get.name(card) == "sha" || get.name(card) == "shan")
          );
        })
        .set("ai", function (card) {
          const player = get.event().player;
  ...
  shzhengxuan: {
    ...
    async content(event, trigger, player) {
      ...
      if (control) {
        if (control == "进攻") {
          if (!player.storage.shzhengxuan_blocker)
            player.setStorage("shzhengxuan_blocker", [0, 0]);
      ...
      blocker: {
        ...
        intro: {
          content: (storage, player) => {
            var str = ``;
            if (player.storage.shzhengxuan_blocker[0] > 0)
              str += `<li>本轮内你计算与其他角色距离-${player.storage.shzhengxuan_blocker[0]}`;
            if (player.storage.shzhengxuan_blocker[1] > 0)
              str += `<li>本轮内其他角色计算与你距离+${player.storage.shzhengxuan_blocker[1]}`;
            return str;
          },
        },
        sub: true,
  ...
  shjilang: {
    ...
    filter: function (event, player) {
      return game.hasPlayer((current) =>
        lib.skill.shjilang.filterTarget(null, player, current),
      );
    },
    filterTarget: function (card, player, target) {
  ...
  shfanjiang: {
    ...
    check: function (event, player) {
      const num = game
        .findPlayer((current) => current.isMaxHandcard())
        .countCards("h");
    ...
    async content(event, trigger, player) {
      ...
      const { control } = await player
        ...
        .set("ai", function () {
          const num = game
            .findPlayer((current) => current.isMinHandcard())
            .countCards("h");
          if (list.includes("选项二") && player.countCards("h") - num < 2)
            return "选项二";
          return list[0];
        })
        .forResult();
      ...
      draw: {
        ...
        async content(event, trigger, player) {
          ...
          const result = await player
            .chooseUseTarget(
              card,
              get.prompt("shdengfeng"),
              "跳过下个回合的" +
                phasename +
                "视为使用一张【决斗】，然后将此阶段摸牌数改为你已损失体力值。",
              false,
            )
            .set("ai", function (target) {
              const player = get.event().player;
  ...
  shdengfeng1: {
    ...
    popup: false,
    async content(event, trigger, player) {
      if (!player.storage.shdengfeng_Skips)
        player.storage.shdengfeng_Skips = [];
      if (!player.storage.shdengfeng_nextSkips)
        player.storage.shdengfeng_nextSkips = [];
      player.storage.shdengfeng_Skips.addArray(
        player.storage.shdengfeng_nextSkips,
      );
      player.storage.shdengfeng_nextSkips = [];
  ...
  shdengfeng3: {
    ...
    intro: {
      content: (storage, player) => {
        ...
        if (
          player.storage.shdengfeng_Skips &&
          player.storage.shdengfeng_Skips.length
        )
          str +=
            "<li>跳过本回合的" +
            get.translation(player.storage.shdengfeng_Skips);
        if (
          player.storage.shdengfeng_nextSkips &&
          player.storage.shdengfeng_nextSkips.length
        )
          str +=
            "<li>跳过下回合的" +
            get.translation(player.storage.shdengfeng_nextSkips);
        return str;
      },
    },
    trigger: {
      ...
      if (num > 0) {
        const result = await target
          .chooseCard(
            ...
            "交给" +
              get.translation(player) +
              get.translation(cmpsuits) +
              "花色的牌各一张并令其本回合虬劲失效，否则你受到其1点雷电伤害",
            function (card) {
              if (ui.selected.cards.length) {
                for (var i of ui.selected.cards) {
                  if (get.suit(card) == get.suit(i)) return false;
  ...
  shduanming: {
    ...
    ai: {
      order: function () {
        return (
          Math.max(
            get.order({ name: "sha", nature: "thunder" }),
            get.order({ name: "sha" }),
          ) + 0.7
        );
      },
    ...
    mod: {
      selectTarget: function (card, player, range) {
        if (card.storage && card.storage.shduanming && range[1] != -1)
          range[1]++;
      },
      targetInRange: function (card) {
    ...
    subSkill: {
      miss: {
        ...
        filter: function (event, player) {
          if (
            !event.targets ||
            !event.targets.some((current) => !current.isLinked())
          )
            return false;
  ...
  shzangliu: {
    ...
    filter: function (event, player) {
      return game.hasPlayer(
        (current) => !current.hasSkill("nzry_jieying") && current.isLinked(),
      );
    },
    filterTarget: function (card, player, target) {
      if (!ui.selected.targets.length)
        return !target.hasSkill("nzry_jieying") && target.isLinked();
  ...
  shenyu: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseCard(
          "he",
          get.prompt("shenyu", trigger.player),
          [1, 3],
          "将任意张类型不同的牌交给" + get.translation(trigger.player),
          function (card) {
            if (!ui.selected.cards.length) return true;
            var type = get.type2(card);
            for (var i of ui.selected.cards) {
  ...
  shyiduo: {
    ...
    filterTarget: function (card, player, target) {
      return (
        player.storage.shenyu_targets.includes(target) &&
        target.hasUseTarget({ name: "juedou", isCard: true })
      );
    },
    async content(event, trigger, player) {
    ...
    async content(event, trigger, player) {
      ...
      await target
        ...
        .set("ai", (card) => {
          ...
          if (player.countCards("h") - ui.selected.cards.length > num)
            return 250 - get.value(card);
          return 0;
        })
        .set("complexCard", true);
      var num = 0;
  ...
  shcaiyin: {
    ...
    filter(event, player) {
      return (
        player.countCards("h") + get.centralDisCards().length > 0 &&
        !player.hasSkill("shcaiyin_yearban")
      );
    },
    check: function (event, player) {
  ...
  shluanji: {
    ...
    subSkill: {
          ...
          if (event.triggername == "useCard") {
            if (!trigger.targets || !trigger.targets.length)
              game.playAudio(
                "..",
                "extension",
                "水泊娘山/audio/skill/shluanji2.mp3",
              );
            trigger.directHit.addArray(targets);
          } else {
            if (trigger.isFirstTarget && get.type(trigger.card) == "trick")
              game.playAudio(
                "..",
                "extension",
                "水泊娘山/audio/skill/shluanji2.mp3",
              );
            else if (targets.includes(trigger.target))
              game.playAudio(
                "..",
                "extension",
                "水泊娘山/audio/skill/shluanji2.mp3",
            ...
            if (trigger.player == player)
              trigger.getParent("useCard").directHit.addArray(targets);
          }
          //game.log(targets);
        },
        ai: {
      ...
      damage: {
        ...
        filter: function (event, player) {
          return (
            player == _status.currentPhase && event.player.countCards("e") > 0
          );
        },
        content: function () {
          game.playAudio(
            "..",
            "extension",
            "水泊娘山/audio/skill/shluanji1.mp3",
          );
          trigger.num++;
        },
        sub: true,
  ...
  shluanjiAudio: {
    ...
    content: function () {
      ...
      game.playAudio("..", "extension", "水泊娘山/audio/skill/shluanji1.mp3");
    },
  },
  shhuaibi: {
    init: function (player) {
    ...
    async content(event, trigger, player) {
      ...
      for (let card of cards) {
        ...
        if (
          (name == "shgoulianqiang" || name == "shsaitangni") &&
          !list.includes(name)
        ) {
          list.add(name);
      ...
      aiOrder: function (player, card, num) {
        if (typeof card == "object") {
          if (
            ...
            player.countCards("h", (card2) => !get.is.shownCard(card2))
          ) {
            if (
              player._shantu_mark.name == "暗" &&
              !player.countCards("h", { name: "ying" })
            )
              return num + 1;
            if (get.type(card) == "equip" || player.getUseValue(card) < 17)
              return Math.max(0, num / 10 - 10) + 0.5;
            return num / 5 + 1;
          }
        }
      },
    ...
    phaseDiscard: function () {
      ...
      player.chooseToDiscard(num, true).ai = function (card) {
        ...
        if (
          player.countCards("h", (card) => {
            return (
              get.name(card) == "ying" && !ui.selected.cards.includes(card)
            );
          }) == 1 &&
          get.name(card) == "ying"
        )
  ...
  shguzhi_view: {
    ...
    filter: function (event, player) {
      if (
        !player.countCards("h", (card) => !get.is.shownCard(card)) ||
        !player.countCards("h", (card) => get.is.shownCard(card))
      )
        return false;
      ...
      for (var i of cards) {
        ...
        if (
          ...
          event.filterCard(
            get.autoViewAs({ name: get.name(i) }, "unsure"),
            player,
            event,
          )
        )
    ...
    hiddenCard: function (player, name) {
      ...
      return (
        (type == "basic" || type == "trick") &&
        player.countCards("h", (card) => !get.is.shownCard(card)) > 0
      );
    },
    ai: {
      fireAttack: true,
      ...
      order: function (item, player) {
        ...
        for (var card of cards) {
          if (
            get.type(card) == "equip" ||
            get.type(card) == "delay" ||
            list1.includes(get.name(card))
          )
            continue;
          if (
            player.getUseValue({ name: get.name(card), isCard: true }) >
            player.getUseValue(maxUseValue)
          )
            maxUseValue = { name: get.name(card), isCard: true };
        }
        if (player.getUseValue(maxUseValue) > 5)
          return get.order(maxUseValue) + 0.1;
        return 0;
      },
      result: {
        player: function (player) {
    ...
    subSkill: {
      hideShown: {
        ...
        filter: function (event, player) {
          return (
            event.skill == "shguzhi_view_backup" &&
            player.countCards("h", (card) => get.is.shownCard(card))
          );
        },
        async content(event, trigger, player) {
    ...
    filter(event, player, name) {
      if (player._shantu_mark.name == "暗") {
        ...
        return (
          evt &&
          evt.hs &&
          evt.hs.filter((h) => get.name(h, false) == "ying").length &&
          !player.countCards("h", (card) => get.name(card, false) == "ying")
        );
      }
    },
    async content(event, trigger, player) {
      if (player._shantu_mark.name == "暗") {
        game.playAudio("..", "extension", "水泊娘山/audio/skill/shantu1.mp3");
        await player.gain(lib.card.ying.getYing(2), "gain2");
      } else {
        game.playAudio("..", "extension", "水泊娘山/audio/skill/shantu2.mp3");
        ...
        content: function () {
          ...
          if (result.color == "black" && player._shantu_mark.name == "明") {
            game.broadcastAll(function (player) {
              ...
              player._shantu_mark.info.content =
                "你失去明置手牌后，你获得两张【影】";
            }, player);
          } else if (
            result.color == "red" &&
  ...
  shlieri: {
    ...
    filter: function (event, player) {
      return (
        ...
        player.countCards("h", lib.filter.cardRecastable) > 0
      );
    },
    async content(event, trigger, player) {
      ...
      const { bool, cards } = await player
        .chooseCard(
          "h",
          "重铸至多半数手牌",
          [1, Math.floor(player.countCards("h") / 2)],
          lib.filter.cardRecastable,
          true,
        )
        .set("ai", function (card) {
      ...
      jiu: {
        ...
        filter: function (event, player) {
          ...
          return event.targets.filter(
            (current) => current != player && current.isIn(),
          ).length;
        },
        async content(event, trigger, player) {
          const targets = trigger.targets
            .filter((current) => current != player && current.isIn())
            .sortBySeat();
          for (const current of targets) {
            const result = await current
              .chooseToUse({
  ...
  shyuyin2: {
    ...
    mod: {
      cardUsable: function (card, player, num) {
        if (card.name == "jiu" && player != _status.currentPhase)
          return Infinity;
      },
    },
    enable: "chooseToUse",
    ...
    chooseButton: {
      dialog(event, player) {
        ...
        for (var source of players) {
          ...
          if (cards.length) {
            ...
            if (
              source == player ||
              player.hasSkillTag("viewHandcard", null, source, true)
            )
              dialog.add(cards);
      ...
      check(button) {
        ...
        if (lib.skill.shdiaoman.isVisable(player, source, card)) {
          if (get.color(card) == "black") {
            return +(
              ...
              !player.countCards(
                "h",
                (c) => get.name(c) === "jiu" || get.name(c) === "tao",
              ).length
            );
          }
          return get.attitude(player, source) > 1
            ? 7 - get.value(card, source)
            : get.value(card, source);
        }
        if (
          !!_status.currentPhase &&
          _status.currentPhase.hasSkill("shlieri_jiu") &&
          _status.currentPhase != player
        ) {
          if (player == source && get.color(button.link) == "black") return 0;
            ...
            (player.countCards(
              "h",
              (card) => get.name(card) == "jiu" || get.name(card) == "tao",
            ) ||
              (player.hasSkill("shlieri") &&
        ...
        return (
          player.getUseValue(
            get.autoViewAs({ name: "jiu" }, [button.link]),
            null,
            true,
        ...
        return {
          ...
          precontent() {
            ...
            if (get.color(card, false) == "black")
              player.addTempSkill("shyuyin2_unUR", { player: "phaseAfter" });
          },
        };
      },
    },
          ...
          sources.filter((source) => {
            return (
              source.countCards("h", (card) => {
                return (
                  lib.skill.shdiaoman.isVisable(player, source, card) &&
                  get.color(card) == "red"
                );
              }) > 0
            );
  ...
  shfengxuan: {
    ...
    filter(event, player) {
      ...
      return (
        evt.card && evt.card.name == "sha" && evt.player.hasSkill("shfengxuan")
      );
    },
    forced: true,
    async content(event, trigger, player) {
      ...
      if (trigger.player.getPrevious() != trigger.player.getNext())
        targets.add(trigger.player.getNext());
          ...
          const result = await target
            ...
            .set("ai1", () => {
              ...
              if (event.source.hasSkill("shyishi")) {
                if (
                  get.attitude(event.player, event.source) < 0 &&
                  (get.attitude(event.player, event.target) > 2 ||
                    (get.attitude(event.player, event.target) < 0 &&
                      event.target.isDamaged()))
                )
                  return 1;
                return -1;
              }
            ...
            .set(
              "skillwarn",
              "替" + get.translation(trigger.player) + "打出一张闪",
            )
            .set("source", player)
            .set("target", trigger.player)
            .forResult();
  ...
  shyishi: {
      ...
      effect: {
        player: function (card, player, target) {
          if (!ui.selected.targets.length) {
            ...
            var damage = 0;
            var damgeTargets = [];
            const allTargets = game.filterPlayer((current) =>
              lib.filter.targetEnabled({ name: "sha" }, player, current),
            );
          ...
          if (ui.selected.targets.length) {
            for (var i of ui.selected.targets) {
              if (get.attitude(player, i) * get.attitude(player, target) <= 0)
                return [-1, -2, -3, -3];
            }
          }
        },
          ...
          effect: {
            target: function (card, player, target) {
              if (card.name == "sha" && player.hasSkill("shyishi")) {
                if (get.attitude(target, player) < 0 || !target.isDamaged())
                  return;
    ...
    shaOtherContent: function () {
      ...
      if (
        ...
        game.playAudio("..", "extension", "水泊娘山/audio/skill/shyishi.mp3");
  ...
  shfengxuan2: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        !trigger.target.hasSkill("shyishi_noshan") &&
        get.attitude(player, trigger.target) >= 0 &&
        player.hasSkill("shyishi")
      ) {
      ...
      if (get.attitude(player, trigger.target) < 0) {
        for (var current of game.filterPlayer()) {
          ...
          if (
            ...
            get.attitude(current, trigger.target) < -1
          )
            current.addTempSkill("shyishi_noshan", "useCardEnd");
        }
      }
    },
  },
  shduiying: {
    ...
    filter: function (event, player) {
      return (
        ...
        player.countCards(
          "h",
          (card) => get.color(card) != get.color(event.card),
        ) &&
        event.targets.filter((target) => target.isIn()).length
      );
    },
    check: function (event, player) {
      var eff = -2;
    ...
    async content(event, trigger, player) {
      ...
      const cards = player.getCards(
        "h",
        (card) => get.color(card) != get.color(trigger.card),
      );
  ...
  shhuisheng: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseTarget(
          get.prompt("shhuisheng"),
          "令一名角色摸两张牌，然后移动其装备区的一张牌",
        )
        .set("ai", (target) => {
      ...
      if (player.canMoveCard(null, true, target))
        player
          ...
          .set(
            "prompt",
            "移动" +
              (target == player ? "你" : get.translation(target)) +
              "装备区一张牌",
          );
    },
  },
  shhufen: {
    ...
    intro: {
      content: function (storage, player, skill) {
        var str = player.storage.shhufen
          ? "出牌阶段开始或结束时，你须获得场上一张装备牌，然后对攻击范围外一名角色造成1点伤害。"
          : "出牌阶段开始或结束时，你须获得场上一张装备牌，然后对攻击范围内一名角色造成1点伤害。";
        return str;
      },
    },
    trigger: {
    ...
    filter: function (event, player) {
      return game.hasPlayer(
        (current) => current.countGainableCards(player, "e") > 0,
      );
    },
    async content(event, trigger, player) {
      ...
      const targets = (
        await player
          ...
          .set("ai", (target) => {
            const player = _status.event.player;
            if (get.attitude(player, target) > 1 && target.hasSkill("xiaoji"))
              return 10;
            if (get.attitude(player, target) < 0) {
              if (
                target.hasCard((card) => {
  ...
  shjianbei: {
    ...
    filter: function (event, player) {
      ...
      if (player._shjianbei_mark.name == "智")
    ...
    async content(event, trigger, player) {
      ...
      const newSkill =
        player._shjianbei_mark.name == "智" ? ["jizhi"] : ["xiaoji"];
  ...
  shzaomeng: {
      ...
      event.result = await player
        .chooseCardTarget({
          ...
          ai2: (target) => {
            const player = _status.event.player;
            if (
              (player.isLinked() || ui.selected.targets.includes(player)) &&
              get.attitude(player, target) > 2
            ) {
              if (player.getPrevious() == target || player.getNext() == target)
                return 1;
    ...
    async content(event, trigger, player) {
      ...
      for (const current of targets) {
        ...
        if (current.isLinked())
          current.addTempSkill("shzaomeng_lock", { player: "linkAfter" });
      }
    },
    subSkill: {
      lock: {
        ...
        async content(event, trigger, player) {
          if (
            player.isLinked() &&
            (!trigger?.getParent("damage") ||
              !trigger?.getParent("damage")?.hasNature("fire"))
          ) {
            trigger.cancel();
          }
        },
  ...
  shsuizhen2: {
    ...
    filter(event, player) {
      ...
      var skillSources = game.filterPlayer((current) =>
        current.hasSkill("shsuizhen"),
      );
      if (!player.storage.shsuizhen2_refused)
        player.storage.shsuizhen2_refused = [];
      for (var skillSource of skillSources) {
        if (get.sameBoatPlayer(skillSource).includes(player)) {
          var players = get
            .sameBoatPlayer(player)
            .filter(
              (current) =>
                current != player &&
                !player.storage.shsuizhen2_refused.includes(current),
            );
      ...
      backup(links, player) {
        return {
          ...
          precontent() {
            ...
            if (player.storage.shsuizhen2_allowed.includes(event.source))
              event.finish();
            ("step 1");
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          if (
            !get.sameBoatPlayer(player).filter((current) => current != player)
              .length
          )
            return 0;
          if (_status.event.dying)
    ...
    subSkill: {
      ...
      allowed: {
        charlotte: true,
        init: function (player) {
          if (!player.storage.shsuizhen2_allowed)
  ...
  shxufa: {
    ...
    async cost(event, trigger, player) {
      ...
      let choiceList = [
        "弃置" + get.cnNumber(X) + "张牌",
        "对一名手牌数不小于你的角色造成1点伤害，然后将手牌摸至" +
          get.cnNumber(X) +
          "张",
      ];
    ...
    async content(event, trigger, player) {
      ...
      if (event.cost_data == "弃牌") {
        ...
        if (player.storage.shxufa_last && player.storage.shxufa_last == 1) {
          if (!player.storage.shxufa_damage)
            player.setStorage("shxufa_damage", 0);
        ...
        const targets = (
          await player
            .chooseTarget(
              ...
              (card, player, target) => {
                return (
                  _status.event.player.countCards("h") <= target.countCards("h")
                );
              },
              true,
            )
    ...
    subSkill: {
      ...
      damage: {
        ...
        content: function () {
          if (trigger.source == player && event.triggername == "damageBegin1")
            trigger.num += player.storage.shxufa_damage;
          if (trigger.player == player && event.triggername == "damageBegin3")
            trigger.num -= player.storage.shxufa_damage;
        },
        sub: true,
      },
  ...
  shhaoyong: {
    ...
    async content(event, trigger, player) {
      ...
      while (
        ...
        player.getExpansions("shhaoyong").length > 0
      ) {
        const cards = player.getExpansions("shhaoyong");
        game.log(cards);
        const { bool, links } = await player
          .chooseButton(["选择一张作为拼点牌", cards], true)
          .forResult();
        if (!bool) continue;
        const result2 = await player
        ...
        if (!result2.tie) {
          if (result2.bool) {
            ...
            if (player.getExpansions("shhaoyong").length > 0)
              await player.chooseToDiscard(
                player.getExpansions("shhaoyong").length,
                true,
                "he",
              );
          }
  ...
  shshaqian: {
          ...
          current.getHistory("lose", function (evt) {
            return (
              ...
              evt.cards2.filter((card) => {
                return (
                  get.number(card, false) > 10 || get.number(card, false) == 1
                );
              }).length > 0
            );
          }).length > 0
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseTarget(
          get.prompt("shshaqian"),
          "对一名本回合失去过字母牌的其他角色造成1点伤害。",
          (card, player, target) => {
            return (
              target != player &&
              target.getHistory("lose", function (evt) {
                return (
                  ...
                  evt.cards2.filter((card) => {
                    return (
                      get.number(card, false) > 10 ||
                      get.number(card, false) == 1
                    );
                  }).length > 0
                );
              }).length > 0
  ...
  shchaqing: {
    ...
    async cost(event, trigger, player) {
      const list = [];
      let choiceList = [
        "与" + get.translation(trigger.player) + "各失去1点体力",
        "与" + get.translation(trigger.player) + "各弃置两张手牌",
      ];
      list.push("失去体力");
      ...
      const result = await player
        ...
        .set("ai", () => {
          ...
          if (get.attitude(player, target) > -1 || player.hp < 2)
            return "cancel2";
          return list[0];
        })
        .set("target", trigger.player)
  ...
  shjianyun: {
    ...
    locked: false,
    async content(event, trigger, player) {
      if (trigger.player.isIn() && player.hp > 0)
        await player.damage(trigger.player);
    },
  },
  shjixian: {
    enable: "phaseUse",
    ...
    async content(event, trigger, player) {
      ...
      if (target.countCards("h") > player.countCards("h")) {
        ...
        if (target.countDiscardableCards(player, "he") > 0) {
          const result = await player
            .discardPlayerCard(target, 2, "he", forced)
            .forResult();
          if (result?.bool && result.links?.length) {
            flag = false;
            if (target.countCards("h") > player.countCards("h"))
      ...
      global: {
        ...
        filter(event, player) {
          ...
          if (
            !game.hasPlayer((current) => {
              return (
                current.hasSkill("shweiji") && current == _status.currentPhase
              );
            }) &&
        ...
        async cost(event, trigger, player) {
          event.result = await player
            .chooseToDiscard(
              get.prompt("shweiji", trigger.player),
              "弃置至多两张手牌并令一名有伪缉的角色摸等量牌，然后取消" +
                get.translation(trigger.player) +
                (trigger.name == "link" ? "的横置" : "的翻面"),
              "h",
              [1, 2],
            )
            .set("ai", function (card) {
              ...
              if (
                ...
                !trigger.player.hasSkill("shjinchong")
              )
                return 8 - get.value(card);
              for (const source of sources) {
                if (get.attitude(player, source) > 2)
                  return 4 - get.value(card);
              }
              return 0;
            })
        ...
        async content(event, trigger, player) {
          ...
          if (sources.length > 1 && trigger.player == _status.currentPhase) {
            const result = await player
              .chooseTarget(
                "令一名有伪缉的角色摸" + get.cnNumber(num) + "张牌",
                (card, player, target) => {
  ...
  shchongjia: {
    ...
    filter(event, player, name) {
      ...
      const bool1 =
        event.player && event.player.isIn() && !event.player.isLinked();
      const bool2 =
        event.source && event.source.isIn() && !event.source.isLinked();
      if (!bool1 && !bool2 && (!event.source || !event.source.isIn()))
        return false;
    ...
    async content(event, trigger, player) {
      var maxChoose =
        trigger.source && trigger.source.isIn() && trigger.source != player
          ? 2
          : 1;
      var canRepeat = trigger.player && trigger.player != player ? true : false;
      var hasRepeat = false;
      const list = [
        `摸一张牌并横置${trigger.source ? get.translation(trigger.source) : "伤害来源"}或${trigger.player ? get.translation(trigger.player) : "受伤角色"}`,
        `对${trigger.source ? get.translation(trigger.source) : "伤害来源"}造成1点雷电伤害`,
      ];
      const links2 = [];
      if (!player.storage.shchongjia) {
        const { bool, links } = await player
          .chooseButton([
            get.prompt("shchongjia"),
            maxChoose == 1 ? "请选择一项" : "请选择一至两项",
          ...
          .set("filterButton", function (button) {
            if (button.link[0] == "摸") {
              const bool1 =
                trigger.source &&
                trigger.source.isIn() &&
                !trigger.source.isLinked();
              const bool2 =
                trigger.player &&
                trigger.player.isIn() &&
                !trigger.player.isLinked();
              return bool1 || bool2;
            }
            return (
              trigger.source &&
              trigger.source.isIn() &&
              !player.storage.shchongjia
            );
          })
          .set("ai", function (button) {
            if (button.link[0] == "摸") {
      ...
      if (links2.includes("摸")) {
        do {
          ...
          if (
            ...
            preTargets.push(trigger.source);
          if (
            trigger.player &&
            trigger.player.isIn() &&
            !trigger.player.isLinked()
          )
            preTargets.push(trigger.player);
  ...
  shxiaxing: {
    ...
    async content(event, trigger, player) {
      ...
      if (player != _status.currentPhase) {
        ...
        if (!_status.currentPhase.storage.shxiaxing_maxHd)
          _status.currentPhase.setStorage("shxiaxing_maxHd", 0);
  ...
  shyijie: {
    ...
    enable: "phaseUse",
    viewAs(cards, player) {
      var source = game.findPlayer(
        (current) => current.hasSkill("shxueshi") && current.hp == 1,
      );
    ...
    filterTarget: function (card, player, target) {
      ...
      return player.canUse(
        { name: name, cards: ui.selected.cards },
        target,
        false,
      );
    },
    check(card) {
      ...
      if (
        _status.event.player.hasSkill("shxiaxing") ||
        game.hasPlayer(
          (target) =>
            _status.event.player.inRange(target) &&
            target.hasSkill("shxiaxing") &&
            get.attitude(_status.event.player, target) > 2,
        )
      )
        num += 3;
      return num - get.value(card);
      ...
      ai: {
        effect: {
          player: function (card, player, target) {
            ...
            if (!player.hasSkill("shxiaxing") && !target.hasSkill("shxiaxing"))
              return;
            if (card.name == "tuixinzhifu") {
              if (player.hasSkill("shxiaxing"))
                return [
                  1,
    ...
    tuixinzhifuContent: function () {
        ...
        if (!hs.length) event.finish();
          ...
          player
            ...
            .set("ai", function (card) {
              ...
              if (
                ["jiedao", "shunshou", "wugu"].includes(get.name(card)) &&
                target.hasSkill("shxiaxing")
              )
                val += 5;
              ...
              if (
                ...
                (get.attitude(player, target) < 1.5 ||
                  target.hasSkill("shyijie"))
              )
                return 0;
              if (get.attitude(player, target) > 1.5)
                return val - get.value(card, player);
      ...
      if (result.bool) {
        player.give(result.cards, target);
        if (event.num < result.cards.length && !target.hasSkill("shyijie"))
          target.addSkills("shyijie");
      }
    },
      ...
      global_ai: {
        ...
        ai: {
          effect: {
            target: function (card, player, target) {
              if (
                card.name == "tuixinzhifu" &&
                get.attitude(target, player) > 0
              ) {
                if (
                  card.storage.shyijie &&
                  !target.hasSkill("shyijie") &&
                  player.hasSkill("shyijie")
                )
                  return [1, get.attitude(target, player)];
                if (
                  target.hasSkill("shxiaxing") &&
                  get.attitude(player, target) > 1
                ) {
  ...
  shxueshi2: {
    mod: {
      attackFrom(from, to, distance) {
        ...
        if (source1 && source1 == from && from.hasSkill("shxueshi"))
          return distance - Math.max(1, source1.hp) + 1;
        else if (source2 && source2 != from && from.group == "liang")
          return distance - Math.max(1, source2.getDamagedHp()) + 1;
      },
    },
  },
  shzhuanfu: {
    ...
    check: function (card) {
      if (
        _status.event.player.getUseValue(card) < 6 ||
        get.type(card) == "equip"
      )
        return 0;
    ...
    async content(event, trigger, player) {
      ...
      if (get.type(card) != "equip") {
        ...
        if (info.allowMultiple != false && !info.notarget)
          yingbian_Effect.add("yingbian_add");
        if (lib.skill.xunshi.isXunshi(card))
          yingbian_Effect.add("yingbian_remove");
        if (name == "sha") yingbian_Effect.add("yingbian_damage");
        yingbian_Effect.add("yingbian_draw");
        if (name == "shan" || name == "wuxie")
          yingbian_Effect.add("yingbian_gain");
        if (!["shan", "tao", "jiu", "du"].includes(name))
          yingbian_Effect.add("yingbian_hit");
        const buttonList1 = yingbian_Condition.map((i) =>
          get.translation(i + "_tag"),
        );
        const buttonList2 = yingbian_Effect.map((i) =>
          get.translation(i + "_tag"),
        );
        ...
        const retranslation = function (translate) {
          ...
          for (var condition of conditions) {
            if (get.translation(condition + "_tag") == translate)
              return condition;
          }
          for (var effect of effects) {
            if (get.translation("yingbian_" + effect + "_tag") == translate)
              return "yingbian_" + effect;
          }
        };
        const { bool, links } = await player
          .chooseButton(
          ...
          .set("filterButton", function (button) {
            if (ui.selected.buttons.length) {
              ...
              var sflag = buttonList1.includes(ui.selected.buttons[0].link)
                ? 1
                : 2;
              return flag != sflag;
            }
        ...
        if (!_status.cardtag[retranslation(links[0])])
          _status.cardtag[retranslation(links[0])] = [];
        if (!_status.cardtag[retranslation(links[1])])
          _status.cardtag[retranslation(links[1])] = [];
        for (var i in _status.cardtag) {
          if (
            !yingbian_Condition.includes(i) &&
            ![
              "yingbian_add",
              "yingbian_remove",
              "yingbian_damage",
        ...
        if (!newList.includes(retranslation(links[0])))
          newList.push(retranslation(links[0]));
        if (!newList.includes(retranslation(links[1])))
          newList.push(retranslation(links[1]));
        card[4] = newList;
      ...
      const { bool, cards, targets } = await player
        ...
        .forResult();
      if (!bool || !targets || !targets.length || !cards || !cards.length)
        return;
      await player.give(cards, targets[0]);
  ...
  shchisheng: {
    ...
    filter: function (event, player) {
      ...
      return (
        player.countCards("he") >= X &&
        player.awakenedSkills.includes("shzhuanfu")
      );
    },
    complexSelect: true,
    check: function (card) {
    ...
    subSkill: {
      global: {
        ...
        prompt:
          "将一张手牌交给有驰声的角色，然后其可以对此牌执行〖篆符〗效果并交给你",
        async content(event, trigger, player) {
          game.playAudio(
            "..",
            "extension",
            "水泊娘山/audio/skill/shchisheng1.mp3",
          );
          const target = event.targets[0];
          const card = event.cards[0];
          const name = get.name(card, false);
          ...
          if (result.bool) {
            if (get.type(card) != "equip") {
              ...
              const retranslation = function (translate) {
                ...
                for (var effect of effects) {
                  if (
                    get.translation("yingbian_" + effect + "_tag") == translate
                  )
                    return "yingbian_" + effect;
                }
              };
              const { bool, links } = await target
                ...
                .set("filterButton", function (button) {
                  if (ui.selected.buttons.length) {
                    var flag = buttonList1.includes(button.link) ? 1 : 2;
                    var sflag = buttonList1.includes(
  ...
  shqinghong: {
      ...
      event.result = await player
        .chooseCardTarget({
          ...
          selectTarget: function () {
            ...
            if (ui.selected.targets) {
              num +=
                ui.selected.targets
                  .map((target) => {
                    return target.countDiscardableCards(
                      _status.event.player,
                      "he",
                    );
                  })
          ...
          ai1: function (card) {
            ...
            if (
              hs.every((card) => selectCards.includes(card)) ||
              es.every((card) => selectCards.includes(card))
            )
              val += 3;
    ...
    async content(event, trigger, player) {
      ...
      for (var i = 0; i < targets.length; i++) {
        ...
        var min = 1;
        var max = Math.min(
          num + i - targets.length + 1,
          target.countDiscardableCards(player, "he"),
        );
        const result = await player
          .discardPlayerCard(
            target == targets[targets.length - 1] ? num : [min, max],
            target,
            "he",
            true,
          )
          .set("ai", function (button) {
            ...
            var target = _status.event.target;
            if (
              (get.position(card) == "h" && target.countCards("h") == 1) ||
              (get.position(card) == "e" && target.countCards("e") == 1)
            )
              return get.value(card) + 60;
    ...
    subSkill: {
      mark: {
        ...
        intro: {
          content: function (storage, player) {
            var str = "";
            if (player.storage.shtaian_num > 0)
              str += "<li>你的攻击范围和手牌上限+" + player.storage.shtaian_num;
      ...
      noshan: {
        ...
        async cost(event, trigger, player) {
          event.result = await player
            .chooseToDiscard(
              "he",
              get.prompt("shlianzhu", trigger.player),
              "弃置一张黑色牌，令" +
                get.translation(trigger.player) +
                "使用的闪无效",
              { color: "black" },
  ...
  shjunjie: {
    ...
    async content(event, trigger, player) {
      ...
      const { bool, links } = await player
        .chooseButton(
          [
            "郡杰：请选择获得下列" + get.cnNumber(num) + "项效果",
    ...
    subSkill: {
      draw: {
        ...
        filter: function (event) {
          return (
            !event.numFixed &&
            event.player.storage.shjunjie &&
            event.player.storage.shjunjie[0] == 1
          );
        },
        content: function () {
          trigger.num++;
      ...
      useSha: {
        ...
        filter: function (event) {
          ...
          return (
            event.player.storage.shjunjie &&
            event.player.storage.shjunjie[1] == 1
          );
        },
        content: function () {
          trigger.effectCount++;
  ...
  shdiaoman1: {
    ...
    async content(event, trigger, player) {
      game.playAudio("..", "extension", "水泊娘山/audio/skill/shdiaoman1.mp3");
      const target = event.targets[0];
      const result = await player
      ...
      if (result.bool) {
        await target.showCards(
          result.links,
          get.translation(target) + "因【刁蛮】展示",
        );
  ...
  shdiaoman2: {
    ...
    async content(event, trigger, player) {
      game.playAudio("..", "extension", "水泊娘山/audio/skill/shdiaoman2.mp3");
      var hs = player.getCards("h");
  ...
  shdiaoman3: {
    ...
    async content(event, trigger, player) {
      ...
      await player
        ...
        .set("filterButton", function (button) {
          return lib.skill.shdiaoman.isVisable(
            get.event().player,
            get.event().target,
            button.link,
          );
    ...
    intro: {
      markcount: (_, player) =>
        [1, 2, 3, 4, 5]
          ...
          .reduce((p, c) => p + c, 0) - player.countMark("shxindang_used"),
      content: (_, player) =>
        "当前剩余发动次数：" +
        ([1, 2, 3, 4, 5]
          .map((i) => player.countEmptySlot(i))
          .reduce((p, c) => p + c, 0) -
          player.countMark("shxindang_used")),
    },
    global: "shxindangx",
    trigger: {
      global: "damageEnd",
    ...
    async cost(event, trigger, player) {
      ...
      if (target.countCards("h")) {
        ...
        if (player.hasSkillTag("viewHandcard", null, target, true))
          ...
          if (target.countCards("h", (card) => !get.is.shownCard(card)))
            dialog.push([
              target.getCards("h").filter((card) => !get.is.shownCard(card)),
              "blank",
            ]);
          if (target.countCards("h", (card) => get.is.shownCard(card)))
            dialog.push(
      ...
      event.result = await player
        ...
        .set("filterButton", (button) => {
          ...
          if (!ui.selected.buttons.length && get.owner(button.link) != target)
            return false;
          if (
            ui.selected.buttons.length &&
        ...
        .set("ai", (button) => {
          ...
          if (att <= 2) {
            if (owner == player) {
              ...
              if (
                lib.skill.shdiaoman.isVisable(player, target, button.link) &&
                ((suit == "heart" && player.isDamaged()) ||
                  get.type2(button.link) == "trick")
              )
                return value * 3;
              return value;
            }
      ...
      if (get.suit(card2) == "heart") {
        ...
        if (list.length) {
          ...
          const result = await player
            ...
            .forResult();
          if (result.bool)
            await player.chooseUseTarget(true, result.links[0][2]);
        }
      }
      if (get.suit(card1) == "heart") {
        ...
        if (list.length) {
          ...
          const result = await trigger.source
            ...
            .forResult();
          if (result.bool)
            await trigger.source.chooseUseTarget(true, result.links[0][2]);
        }
      }
    },
    subSkill: {
  ...
  shxindangx: {
    ...
    filter: function (event, player) {
      if (!player.hasSex("male") || !event.player.hasSkill("shxindang"))
        return false;
      if (
        event.player.countMark("shxindang_used") >=
        [1, 2, 3, 4, 5]
          .map((i) => event.player.countEmptySlot(i))
          .reduce((p, c) => p + c, 0)
      )
        return false;
      ...
      if (get.suit(card1) == "heart") {
        ...
        if (list.length) {
          ...
          const result = await trigger.player
            ...
            .forResult();
          if (result.bool)
            await trigger.player.chooseUseTarget(true, result.links[0][2]);
        }
      }
    },
  },
  shxiongju: {
    ...
    async content(event, trigger, player) {
        ...
        const { bool, cards } = await target
          ...
          .set("ai", (card) => {
            if (get.name(card, false) == "sha") {
              var num =
                _status.event.player.countCards("h", { name: "sha" }) /
                _status.event.source.countCards("h");
      ...
      for (const target2 of targets2) {
        await player.useCard(
          { name: "juedou", isCard: true, storage: { shxiongju: true } },
          target2,
        );
      }
    },
    ...
    subSkill: {
      dying: {
        ...
        filter: function (event, player) {
          ...
          return (
            evt && evt.card && evt.card.storage && evt.card.storage.shxiongju
          );
        },
        forced: true,
        popup: false,
  ...
  shfushe: {
    ...
    filter: function (event, player) {
      return (
        player.hasUseTarget({ name: "sha", nature: "stab" }, false) &&
        !player.isTurnedOver()
      );
    },
    check: function (event, player) {
      return game.hasPlayer((target) => {
        return (
          get.effect(
            target,
            { name: "sha", nature: "stab", isCard: true },
            player,
            player,
          ) > 0
        );
    ...
    ai: {
      ...
      ai: {
        effect: {
          ...
          target: function (card, player, target) {
            if (!player) player = _status.event.player;
            if (ui.selected.targets.length && game.filterPlayer().length < 4)
              return [0, 0];
            if (
              get.name(card) == "diaohulishan" &&
              card.storage &&
  ...
  shzhuilie_sha: {
    ...
    filter: function (event, player) {
      ...
      return (
        event.card &&
        event.card.storage &&
        event.card.storage.shzhuilie &&
        event.card.name == "diaohulishan"
      );
    },
    direct: true,
    async content(event, trigger, player) {
    ...
    subSkill: {
      target: {
        ...
        filter: function (event, player) {
          if (event.targets.length != 1 || !event.target.countCards("he"))
            return false;
          return (
            event.card.name == "sha" &&
            event.card.storage &&
        ...
        async content(event, trigger, player) {
          await player.gainPlayerCard(
            "he",
            trigger.target,
            Math.min(2, trigger.target.countCards("he")),
            true,
          );
        },
        sub: true,
  ...
  shjuantu: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        evt.cards2.length &&
        evt.cards2.length <=
          game.filterPlayer((current) => current != player).length
      );
    },
    async cost(event, trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          num,
          get.prompt("shjuantu"),
          "令" + get.cnNumber(num) + "名其他角色摸或弃置一张牌。",
          (card, player, target) => {
            return target != player;
          },
        )
        .set("ai", (target) => {
          if (get.attitude(_status.event.player, target) < 0)
            return (
              get.effect(target, { name: "guohe_copy2" }, player, player) / 1.2
            );
          return get.effect(target, { name: "draw" }, player, player);
        })
        .forResult();
    ...
    async content(event, trigger, player) {
      player.addTempSkill("shjuantu_used", [
        "phaseZhunbeiAfter",
        "phaseDrawAfter",
        "phaseJudgeAfter",
        "phaseUseAfter",
        ...
        do {
          const { bool, links } =
            cards.length == 1
              ? { links: cards.slice(0), bool: true }
              : await player
                  .chooseCardButton("卷土：请选择要分配的牌", true, cards, [
                    1,
                    Math.min(cards.length, assign_Max),
                  ])
                  .set("ai", () => {
                    if (ui.selected.buttons.length == 0) return 1;
                    return 0;
  ...
  shjuxian: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseCard(
          "he",
          get.prompt("shjuxian", trigger.player),
          "将一张装备牌置入" +
            get.translation(trigger.player) +
            "的装备区并摸一张牌",
          function (card) {
            return (
              get.type(card) == "equip" &&
              _status.event.target.canEquip(card, true)
            );
          },
        )
        .set("ai", function (card) {
    ...
    async content(event, trigger, player) {
      ...
      await player.draw();
      if (subType == "equip1") {
        if (get.mode() == "doudizhu" && target.identity != "fan")
          skills.add("bahu");
        else skills.add("jsrgbahu");
      } else if (subType == "equip2") skills.add("xiangle");
      if (skills.length)
        target.addTempSkills(skills, { player: "phaseBefore" });
    },
  },
  /*shguiyin: {
                    audio:'ext:水泊娘山/character/audio/skill:2',
  ...
  shguiyin: {
    ...
    init(player, skill) {
      player.addExtraEquip(
        ...
        "danshutiequan",
        true,
        (player) => player.hasEmptySlot(5) && lib.card.danshutiequan,
      );
    },
  ...
  shzuoyao: {
    ...
    enable: "chooseToUse",
    prompt:
      "将【杀】当做【无懈可击】，【闪】当做【无中生有】，【桃】当做【闪电】使用",
    viewAs(cards, player) {
      if (cards.length) {
        var name = false;
        switch (get.name(cards[0], player)) {
    ...
    check(card) {
      if (
        _status.event.getParent().name == "shfengqi" &&
        get.name(card) != "sha"
      )
    ...
    filterCard(card, player, event) {
      ...
      if (
        name == "shan" &&
        filter({ name: "wuzhong", cards: [card] }, player, event)
      )
        return true;
      if (
        name == "tao" &&
        filter({ name: "shandian", cards: [card] }, player, event)
      )
        return true;
      return false;
    },
    filter(event, player) {
      ...
      if (
        filter(get.autoViewAs({ name: "wuxie" }, [sha]), player, event) &&
        player.countCards("hs", { name: "sha" })
      )
        return true;
      if (
        filter(get.autoViewAs({ name: "wuzhong" }, [shan]), player, event) &&
        player.countCards("hs", { name: "shan" })
      )
        return true;
      if (
        filter(get.autoViewAs({ name: "shandian" }, [tao]), player, event) &&
      ...
      global: {
        ...
        mod: {
          cardEnabled(card, player) {
            var list = [];
            game.countPlayer(function (current) {
              if (current.storage.shzuoyao && current.storage.shzuoyao.length)
                list.addArray(current.storage.shzuoyao);
            });
    ...
    async content(event, trigger, player) {
      ...
      do {
        ...
        if (result.bool) {
          ...
          if (
            player.countCards("h") > h ||
            player.countCards("e") > e ||
            player.countCards("j") > j
          ) {
            bool = true;
            num++;
  ...
  shyangsha: {
    ...
    hiddenSkill: true,
    filter: function (event, player) {
      return event.toShow?.some((i) =>
        get.character(i).skills?.includes("shyangsha"),
      );
    },
    forced: true,
    async content(event, trigger, player) {
      ...
      event.result = await player
        .chooseCardTarget({
          ...
          ai1: function (card) {
            ...
            var val =
              get.name(card) == "sha" ? get.value(card) - 1 : get.value(card);
          ...
          ai2: function (target) {
            ...
            return get.effect(
              target,
              { name: "sha", cards: ui.selected.cards },
              player,
              player,
            );
          },
    ...
    filter: function (event, player) {
      ...
      for (var current of game.filterPlayer()) {
        ...
        if (
          ...
          evt.js &&
          evt.js.length &&
          evt.js.some((card) => get.type(card, false) == "equip")
        )
          cards.addArray(
            evt.js.filter((card) => get.type(card, false) == "equip"),
          );
        if (
          evt &&
          evt.es &&
          evt.es.length &&
          evt.es.some((card) => get.type(card, false) == "equip")
        )
          cards.addArray(
            evt.es.filter((card) => get.type(card, false) == "equip"),
          );
      }
      return (
        cards.length &&
        ...
        target: function (player, target, card) {
          ...
          if (att > 0) {
            if (player.countCards("h") >= target.countCards("h") + 2)
              return 2.5;
            else if (player.hp >= target.hp - 1 && target.hp > 1)
              return (2 * (target.hp - 0.91)) / target.maxHp;
            return -1;
          } else {
            if (player.countCards("h") >= target.countCards("h") + 2) return 0;
            else if (player.hp >= target.hp - 1)
              return 2 * ((target.countCards("h") + 2) * 0.11 - 1) + att;
            return -2.5 + att;
          }
        },
      },
    ...
    async content(event, trigger, player) {
      ...
      for (let target of targets) {
        const result = await target
          ...
          .set("ai", function (card) {
            if (get.attitude(_status.event.player, _status.event.source) > 1.5)
              return (
                50 -
                get.value(card) +
                (get.suit(card, false) == "spade" ? 50 : 0)
              );
      ...
      if (player.storage.shqinye && player.storage.shqinye.length)
        player.addTempSkills("shqinye2", "roundStart");
    },
    subSkill: {
      ai: {
        charlotte: true,
            ...
            target: function (card, player, target) {
              if (
                ...
                game.hasPlayer((source) => {
                  return (
                    source.hasSkill("shqinye") &&
                    source.storage.shqinye &&
                    source.storage.shqinye.includes(target)
                  );
                })
              ) {
  ...
  shqinye2: {
    ...
    filter: function (event, player) {
      return (
        player.storage.shqinye && player.storage.shqinye.includes(event.target)
      );
    },
    content: function () {
  ...
  shjiaolan: {
    ...
    async content(event, trigger, player) {
      ...
      do {
        const { bool, links } = await player
          .chooseCardButton(
            "搅澜：请选择要分配的牌",
            cards.filter((card) => get.type(card) == "basic").length > 0,
            cards,
            [1, cards.length],
          )
          .set("filterButton", function (button) {
        ...
        const targets = (
          await player
            ...
            .set("ai", function (target) {
              var att = get.attitude(get.event().player, target);
              var bool =
                get.name(links[0]) == "shan" || get.name(links[0]) == "sha";
              if (bool) {
                var val = 0;
                if (
                  !givetoEnemy.includes(target) &&
                  !target.hasCard(
                    (card) => card.hasGaintag("visible_shjiaolan"),
                    "h",
                  )
                )
                  val += 1;
                ...
                if (
                  lib.skill.shpengshuai.hasPhase(target) ||
                  get.name(card) == "shan"
                )
                  val += 1;
                ...
                if (!lib.skill.shpengshuai.hasPhase(target) || target == player)
                  val += 1.8;
        ...
        if (targets.length) {
          ...
          if (get.attitude(player, targets[0]) < 0)
            givetoEnemy.push(targets[0]);
          map[id].addArray(event.togive);
        }
      } while (
        cards.filter((card) => get.type(card, false) == "basic").length > 0
      );
      if (_status.connectMode) {
        game.broadcastAll(function () {
          delete _status.noclearcountdown;
    ...
    subSkill: {
      damage: {
        ...
        filter(event, player) {
          return game.hasPlayer((current) => {
            return current.countCards("h", (card) =>
              card.hasGaintag("visible_shjiaolan"),
            );
          });
        },
        async content(event, trigger, player) {
  ...
  shzonglang: {
    ...
    onRound(event) {
      return (
        (event.relatedEvent || event.getParent(2)).name != "shzonglang_phase"
      );
    },
  ...
  shzonglang_phase: {
    ...
    check(source, player) {
      const players = game.players
        ...
        .sort(
          (a, b) => parseInt(a.dataset.position) - parseInt(b.dataset.position),
        );
    ...
    subSkill: {
      start: {
        ...
        filter: function (event, player) {
          if (event.player.seatNum >= player.seatNum || event.shzonglangs_phase)
            return false;
          return (
            !event.skill &&
            event.player.seatNum == player.getStorage("shzonglang_phase")
          );
        },
        forced: true,
        popup: false,
      ...
      over: {
        ...
        filter(event, player) {
          var targets = game.players
            ...
            .filter(
              (current) =>
                current.seatNum == player.getStorage("shzonglang_phase"),
            );
          ...
          return (
            targets &&
            lib.skill.shzonglang_phase.check(event.player, targets[0]) &&
            !targets[0].isAlive()
          );
        },
        forced: true,
  ...
  shnilinx: {
    ...
    filterCard: function (card) {
      return (
        get.itemtype(card) == "card" &&
        get.name(card) == _status.event._proto[0]
      );
    },
    position: "hs",
    viewAs: {
  ...
  shjianwu: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("ai", function (card) {
          ...
          if (
            player.storage.shjianwu.length > 1 &&
            get.type2(
              player.storage.shjianwu[player.storage.shjianwu.length - 2],
            ) == "trick" &&
            player.storage.shjianwu[player.storage.shjianwu.length - 1] ==
              "重铸闪" &&
            player.hasValueTarget(cardx, false)
          )
            return get.order(card) + 0.15;
          var validTricks = player.getCards("h", function (c) {
            ...
            var positiveCount = game.countPlayer(function (target) {
              return (
                lib.filter.targetEnabled2(c, player, target) &&
                get.effect(target, c, player, player) > 0
              );
            });
          ...
          if (
            ...
            player.countCards("h", { name: "sha" }).length > 1 &&
    ...
    ai: {
      order: function () {
        ...
        var maxOrder =
          0.1 + Math.max(...validTricks.map((card) => get.order(card)));
        ...
        if (player.countCards("h", { name: "shan" }))
          return 10 - Math.min(10, 3 * cnt);
        if (
          player.countCards("h", { name: "sha" }) > 0 &&
          validTricks.length > 0 &&
          last != "打出杀"
        )
          return maxOrder - Math.min(maxOrder, 3 * cnt);
        if (
          !player.countCards("h", { name: "shan" }) &&
          player.countCards("h", { name: "sha" }).length > 1 &&
          !player.getCardUsable("sha")
        )
          return get.order({ name: "sha" }) - Math.min(5, 3 * cnt);
    ...
    mod: {
      aiOrder: function (player, card, num) {
        if (
          typeof card !== "object" ||
          get.type2(card) !== "trick" ||
          !player.storage.shjianwu.length
        )
          return num;
        var history = player.storage.shjianwu;
        var last = history[history.length - 1];
    ...
    subSkill: {
      ...
      combo13: {
        ...
        filter(event, player) {
          if (get.type(event.card) != "trick" || !event.card.storage.shjianwu)
            return false;
          return game.hasPlayer(
            (current) =>
              !event.targets.includes(current) &&
        ...
        content() {
          if ([true, false].randomGet())
            game.playAudio(
              "..",
              "extension",
              "水泊娘山/audio/skill/shcheying1.mp3",
            );
          else
            game.playAudio(
              "..",
              "extension",
              "水泊娘山/audio/skill/shcheying2.mp3",
            );
          player.draw();
        },
        sub: true,
  ...
  shqizhen_func: {
    ...
    error: function (player) {
      if (
        !player.storage.shqizhen_subnum ||
        !player.storage.shqizhen_opqueue.length
      )
        lib.skill.shqizhen_func.reinit(player);
    },
    shqizhen_zhuanhuanji: async function (event, trigger, player) {
      ...
      if (!storage) {
        ...
        for (var i = 0; i < lib.inpile.length; i++) {
          ...
          if (name == "sha") {
            if (!player.storage.shqizhen_opqueue.includes(name))
              list.push(["基本", "", "sha"]);
          } else if (
            get.type(name) == "trick" &&
            !player.storage.shqizhen_opqueue.includes(name)
          )
            list.push(["锦囊", "", name]);
          else if (
            get.type(name) == "basic" &&
    ...
    opsort: function (player, vcards, i) {
      const weights = vcards.map((vcard) =>
        player.getUseValue({ name: vcard[2] }),
      );
      const maxWeight = Math.max(...weights);
      const maxIndex = vcards.findIndex(
        (vcard) => player.getUseValue({ name: vcard[2] }) === maxWeight,
      );
      const maxVcard = vcards[maxIndex];
      const remaining = vcards.filter((vcard, index) => index !== maxIndex);
      remaining.sort(
        (a, b) =>
          player.getUseValue({ name: b[2] }) -
          player.getUseValue({ name: a[2] }),
      );
  ...
  shqizhen: {
    ...
    intro: {
      markcount: (storage, player) =>
        1 + ((storage || 0) % player.storage.shqizhen_subnum),
      content(storage, player) {
        ...
        return (
          ...
          "的倍数的牌置于牌堆两端，视为使用一张" +
          "【" +
          get.translation(player.storage.shqizhen_opqueue[num - 1]) +
          "】。"
        );
      },
    },
    ...
    hiddenCard: function (player, name) {
      var storage = player.storage.shqizhen;
      var name2 =
        player.storage.shqizhen_opqueue[
          (storage || 0) % player.storage.shqizhen_subnum
        ];
      if (name == name2) return true;
      return false;
    },
    ...
    check(card) {
      ...
      if (precards.includes(card) && get.value(precards) < 9)
        return 7 - get.value(card);
      return 0;
    },
    popname: true,
    ...
    async precontent(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        player.lose(cards2, ui.cardPile).set("top", top).insert_index =
          function (event, card) {
      ...
      await lib.skill.shqizhen_func.shqizhen_zhuanhuanji(
        event,
        trigger,
        player,
      );
  ...
  shqizhen_resort: {
    ...
    async content(event, trigger, player) {
      var list = [];
      game.playAudio("..", "extension", "水泊娘山/audio/skill/shqizhen3.mp3");
      ...
      const result = await player
        ...
        .set("processAI", function () {
          ...
          return [
            lib.skill.shqizhen_func.opsort(
              player,
              listx,
              (player.storage.shqizhen || 0) % player.storage.shqizhen_subnum,
            ),
  ...
  shyingxi: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseBool(
          "是否失去1点体力？展示牌堆底五张牌，无距离限制的使用其中所有【杀】且分配剩余基本牌。",
        )
        .set("ai", function () {
          ...
          var cards = [];
          for (
            var i = ui.cardPile.childNodes.length - 1;
            i >= ui.cardPile.childNodes.length - num;
            i--
          ) {
      ...
      do {
        const { bool, links } = await player
          .chooseCardButton("鹰袭：请选择要分配的牌", basic.length > 0, basic, [
            1,
            basic.length,
          ])
          .set("ai", () => {
  ...
  shqinyu: {
    ...
    viewAsFilter: function (player) {
      if (
        !player.countCards(
          "hs",
          (card) => get.name(card) == "sha" && get.color(card) == "black",
        )
      )
        return false;
      ...
      player
        ...
        .then(() => {
          if (trigger.target.sex == "female") {
            trigger.target
              ...
              .set("ai", function () {
                return (
                  get.attitude(_status.event.player, _status.event.source) > 2.5
                );
              })
              .set("source", player);
          } else event.finish();
        ...
        .then(() => {
          if (result.bool) {
            ...
            if (!trigger.card.storage.shqinyu)
              trigger.card.storage.shqinyu = [trigger.target];
    ...
    subSkill: {
      ...
      toulianghuanzhu: {
        ...
        filter: function (event, player) {
          return (
            event.card.storage &&
            event.card.storage.shqinyu &&
            event.card.storage.shqinyu.includes(event.target)
          );
        },
        content: function () {
    ...
    toulianghuanzhuContent: function () {
      ...
      if (result.bool) {
        ...
        player.chooseCard(
          ...
          true,
          "用任意张手牌替换" + get.translation(event.cards),
        ).ai = function (card) {
          return -get.value(card);
        };
      } else {
  ...
  shshibi: {
    ...
    filter(event, player) {
      return (
        game.hasPlayer((current) => current != player && current.isMaxHp()) &&
        player.countCards("he")
      );
    },
    async cost(event, trigger, player) {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        ...
        .set("choiceList", [
          ...
          "令" + get.translation(player) + "从弃牌堆获得一张【杀】",
        ])
        .set("ai", function () {
          if (get.attitude(get.event().player, get.event().target) < 2)
            return "cancel2";
  ...
  shchimou: {
    ...
    mod: {
      ...
      cardnature(card, player) {
        if (
          get.type(card, "trick", false) == "trick" &&
          get.color(card) == "red"
        )
          return "fire";
      },
      targetInRange: function (card, player, target) {
        if (!card.cards || card.cards.length != 1) return;
        if (
          ["trick", "delay"].includes(lib.card[card.cards[0].name].type) &&
          get.color(card) == "red"
        )
          return true;
      },
  ...
  shlaiwuying: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", function (card) {
          if (
            !get.event().player.countCards("hs", { name: "sha" }) &&
            get.event().player.countCards("hs", { name: "shan" })
          )
            return 0;
          if (get.attitude(get.event().player, get.event().source) >= 0)
  ...
  shquwuzong: {
    ...
    filter(event, player) {
      var suits = lib.skill.shmifeng
        .getSuit()
        .filter((st) => lib.suit.includes(st));
      return suits.length + 1 == lib.suit.length;
    },
    direct: true,
    async content(event, trigger, player) {
      const suits = lib.skill.shmifeng
        .getSuit()
        .filter((st) => lib.suit.includes(st));
      ...
      const result = await player
        .chooseToUse({
          ...
          ai1: function (card) {
            if (
              lib.suit.includes(get.suit(card, false)) &&
              !suits.includes(get.suit(card, false))
            )
              return get.order(card) + 1;
            return get.order(card);
          },
        })
        .forResult();
      if (result.bool) {
        ...
        if (lib.skill.shmifeng.getSuit().length == 4 && player.isIn()) {
          const num = lib.skill.shmifeng
            .getSuit()
            .filter((st) => st != suit).length;
          const cards = get.centralDisCards();
          const result2 = await player
            .chooseButton(
              [
    ...
    mod: {
      ...
      cardDiscardable: function (card, player, name) {
        if (
          name == "phaseDiscard" &&
          get.is.shownCard(card) &&
          card.hasGaintag("visible_shxuxun")
        )
          return false;
      },
    },
  ...
  shxiaojue: {
    ...
    async content(event, trigger, player) {
      ...
      await player
        ...
        .set("ai", (card) => {
          var color =
            get.damageEffect(targetx, player, player) > 0 ? "black" : "red";
          if (get.color(card) == color) return 3 + Math.random();
          return Math.random();
        })
        .set("aiCard", (target) => {
          var color =
            get.damageEffect(targetx, target, target) > 0 ? "black" : "red";
  ...
  shbitang: {
    ...
    async content(event, trigger, player) {
      var choice =
        trigger.player.identity == "mingzhong"
          ? "zhong"
      ...
      game.log(
        ...
        trigger.player,
        "的",
        `${color}${get.translation(trigger.player.identity + "2")}`,
        "身份牌",
      );
          ...
          if (player.isUnderControl(true)) {
            dialog.addSmall([
              ...
              (item, type, position, noclick, node) =>
                lib.skill.shbitang.$createButton(
                  item,
                  type,
                  position,
                  noclick,
  ...
  shbitang_zaowang: {
    ...
    filter(event, player) {
      return (
        player.storage?.shbitang?.includes("zhong") ||
        player.storage?.shbitang?.includes("mingzhong")
      );
    },
    prompt:
      "出牌阶段，你可以移去一张“忠臣”身份牌，令一名角色加1点体力上限，回复1点体力并摸三张牌，且获得如下效果：主公死亡时，若其身份为忠臣，则其和主公交换身份牌；其死亡时，若其身份为反贼且伤害来源的身份为主公或忠臣，则以主忠胜利结束本局游戏。",
    content() {
  ...
  shbitang_luanwu: {
    ...
    prompt:
      "出牌阶段，你可以移去一张“内奸”身份牌，你可令除你外的所有角色依次对与其距离最近的另一名角色使用一张【杀】，否则失去1点体力。",
    async content(event, trigger, player) {
  ...
  shwujin: {
    ...
    checkUsed(player, historys) {
      ...
      return (
        get.subtype(historys[0].card) == "equip1" &&
        get.tag(historys[1].card, "damage") > 0.5
      );
    },
    onremove(player, skill) {
    ...
    filter(event, player) {
      if (
        !player.storage.shwujin_comboList ||
        player.storage.shwujin_comboList.length != 3
      )
        return false;
    ...
    subSkill: {
      mark: {
        ...
        async content(event, trigger, player) {
          if (
            lib.skill.shwujin.checkUsed(
              player,
              lib.skill.shwujin.getUsed(player, false),
            )
          ) {
            player.storage.shwujin_comboList = lib.skill.shwujin.getUsed(
              player,
              false,
            );
            ...
            player.storage.shwujin_comboList &&
            player.storage.shwujin_comboList.length == 2
          ) {
            if (
              lib.skill.shwujin.checkUsed(
                player,
  ...
  shrenxia: {
    ...
    filter(event, player) {
      return (
        (player.inRange(event.player) || player == event.player) &&
        player.countCards("h") != player.getAttackRange()
      );
    },
    async content(event, trigger, player) {
      //const target = trigger.source;
      ...
      if (num > numx) {
        const cards = (
          await player.chooseToDiscard("h", true, num - numx).forResult()
        ).cards;
        if (cards.length >= 1) {
          player
            ...
            .step(async (event, trigger, player) => {
              var ds = Array.from(ui.discardPile.childNodes).filter((i) => {
                var bool =
                  get.tag(i, "damage") > 0.5 &&
                  player.hasUseTarget(i, false, false);
  ...
  shchijun: {
    ...
    async content(event, trigger, player) {
      ...
      if (players1.length && players2.length) {
        const list = ["路径一", "路径二"];
        let choiceList = [
          get.pathTranslation(players1),
          get.pathTranslation(players2),
        ];
        const { control } = await player
          .chooseControl(list)
          .set("choiceList", choiceList)
          .set(
            "prompt",
            "选择一个路径，令该路径上所有角色交给你或" +
              get.translation(another) +
              "一张牌",
          )
          .set("ai", function () {
            var player = _status.event.player;
            ...
            if (2 * (p1 - p2) > players1.length - players2.length)
              return list[0];
            return list[1];
          })
          .forResult();
      ...
      for (const current of Targets) {
        ...
        const { bool, cards, targets } = await current
          .chooseCardTarget({
            ...
            ai1(card) {
              if (
                get.name(card, false) == "shan" ||
                get.name(card, false) == "jiu"
              )
                return 100;
              return 100 - get.value(card);
            },
    ...
    subSkill: {
      damage: {
        ...
        popup: false,
        filter(evt) {
          if (!evt.card || !evt.card.storage || !evt.card.storage.shchijun)
            return false;
    ...
    subSkill: {
      guanshi: {
        ...
        content() {
          ...
          player
            .chooseToDiscard(
              get.prompt2("guanshi_skill"),
              "弃置两张牌，令" + get.translation(trigger.card) + "强制命中",
              2,
              "he",
            )
            .set("ai", function (card) {
        ...
        mod: {
          attackRangeBase(player) {
            ...
            if (typeof num != "number") return;
            return Math.max(
              player.getEquipRange(player.getCards("e")),
              1 - num,
            );
          },
        },
  ...
  shyaoyan: {
    ...
    async precontent(event, trigger, player) {
      ...
      for (var i = 0; i < targets.length; i++) {
        list.push([
          targets[i],
          [game.createCard("du", "heart", Math.floor(Math.random() * 13) + 1)],
        ]);
      }
      await game
        .loseAsync({
  ...
  shdingli: {
    ...
    filter(event, player) {
      ...
      return (
        ["shan", "tao"].includes(event.card.name) &&
        Math.max(x, player.countCards("h")) < player.hp
      );
    },
    async content(event, trigger, player) {
      ...
      audio: {
        ...
        filter(event, player) {
          return (
            ...
            player.countUsed(event.card.name, true) > 1 &&
      ...
      move: {
        ...
        filter: function (event, player) {
          ...
          var tgs = game.filterPlayer((i) => {
            return i.countCards(
              "e",
              (c) => get.name(c) == "shxuantianhunyuanjian",
            );
          });
          ...
          player
            .moveCard(
              ...
              game.filterPlayer((i) => {
                return i.countCards(
                  "e",
                  (card) => get.name(card) == "shxuantianhunyuanjian",
                );
              }),
              game.filterPlayer(),
              (card) => {
  ...
  shyingong: {
    ...
    check: (card) => {
      ...
      return (
        ...
        Math.min(
          6.5,
          2.5 * _status.event.player.countCards("h", { name: "sha" }),
        ) -
        get.value(card)
      );
    },
    ...
    delay: false,
    async content(event, trigger, player) {
      const cur_InRangers = game.filterPlayer((current) =>
        player.inRange(current),
      );
      ...
      for (var current of InRangers) {
        if (!cur_InRangers.includes(current))
          player.storage.shyingong_buff.add(current);
      }
    },
    ai: {
      order: 10,
  ...
  shzhaxi: {
    ...
    async cost(event, trigger, player) {
      let choiceList = [
        "令你使用的下张【杀】伤害+1，直到其他角色对你使用【杀】",
        "令下张以你为目标的【杀】无效且你获得之，直到你使用【杀】",
      ];
      ...
      for (const current of event.targets.sortBySeat()) {
        const result = await player
          ...
          .set("ai", function (button) {
            ...
            if (get.event().target == get.event().player)
          ...
          .forResult();
        if (result?.bool && result.links?.length) {
          if (get.name(result.links[0], false) == "sha")
            sha.add(result.links[0]);
          else if (get.name(result.links[0], false) == "shan")
            player.addTempSkill("shpifeng_buff");
        }
      }
  ...
  shjianhen: {
    ...
    filter(event, player) {
      if (
        !event.card ||
        !event.cards?.length ||
        !event.cards.filter((card) => get.name(card, false) == "sha").length
      )
        return false;
    ...
    async content(event, trigger, player) {
      var cards = [];
      if (trigger.player == player)
        cards = trigger.cards.filter((card) => get.name(card, false) == "sha");
  ...
  shfeihu: {
    ...
    filter(event, player) {
      return (
        player.getExpansions("shjianhen").length > 0 &&
    ...
    chooseButton: {
      dialog(event, player) {
        return ui.create.dialog(
          "飞虎",
          player.getExpansions("shjianhen"),
          "hidden",
        );
      },
      filter(button, player) {
    ...
    ai: {
      ...
      result: {
        player(player) {
          return player.getExpansions("jianhen").length;
        },
      },
      combo: "shjianhen",
    },
  ...
  shhutun: {
    ...
    filter: function (event, player) {
      ...
      return (
        event.card.name == "sha" &&
        lib.suit.includes(get.suit(event.card)) &&
        !suits.includes(get.suit(event.card))
      );
    },
    async content(event, trigger, player) {
      await player.gain(trigger.cards, "gain2");
  ...
  shnongwu: {
    ...
    precontent() {
      ...
      player
        ...
        .filter(
          (evt) => evt.card && evt.card.storage && evt.card.storage.shnongwu,
        )
        .then(() => {
          trigger.num--;
        });
          ...
          for (const current of targets) {
            const result = await current
              ...
              .set("ai", function (card) {
                ...
                if (
                  get.attitude(get.event().player, get.event().target) >= 1 &&
                  (get.event().count > 1 ||
                    get.event().count >= get.event().target.hp)
                ) {
                  if (
                    (get.event().target.getEquip(2) || []).some(
                      (card) => get.name(card) === "baiyin",
                    )
                  )
                    return 0;
                  return val - player.getUseValue(card) / 5;
                } else if (
                  get.attitude(get.event().player, get.event().source) > 1 &&
                  get.event().target.countCards("h") >
                    get.event().source.countCards("h")
                )
                  return (
                    player.getUseValue(card) +
                    (get.name(card) == "sha" ? 3.8 : 0.6)
                  );
                return 0;
              })
  ...
  shxiangma: {
    ...
    filter: (event, player) => {
      ...
      es = es.filter(
        (c) =>
          get.number(c, false) ===
          Math.max(...es.map((c) => get.number(c, false))),
      );
      return (
        es.length &&
        game.filterPlayer((current) => {
          for (var card of es) {
            if (current.canEquip(card)) return true;
          }
          return false;
        }).length
      );
    ...
    chooseButton: {
      ...
      backup(links, player) {
        return {
          card: links[0],
          filterTarget(card, player, target) {
            return target.canEquip(links[0]);
          },
    ...
    async contentx(event, trigger, player) {
      ...
      if (result.bool) {
        await player
          .discardPlayerCard("ej", true, result.targets[0])
          .set("filterButton", function (button) {
            var card = button.link;
            return (
  ...
  shshengxi: {
    ...
    async cost(event, trigger, player) {
      ...
      let choiceList = [
        "令" + get.translation(target) + "进攻距离+1(正面效果)",
        "令" + get.translation(target) + "进攻距离-1(负面效果)",
      ];
      const result = await player
        .chooseControl(list)
        ...
        .set("ai", () => {
          if (get.attitude(get.event().player, get.event().target) > 1)
            return list[0];
          else if (get.attitude(get.event().player, get.event().target) < -0.5)
            return list[1];
          return "cancel2";
        })
        .set("target", target)
    ...
    async content(event, trigger, player) {
      ...
      if (!target.storage.shshengxi_dist)
        target.setStorage("shshengxi_dist", 0);
      ...
      if (
        ...
        es.filter((i) => {
          ...
          return (
            subtype == "equip3" || subtype == "equip4" || subtype == "equip6"
          );
        }).length
      )
        await player.draw(2);
      ...
      if (
        num &&
        !game.filterPlayer((current) => target.inRange(current)).length
      ) {
        const result = await player
    ...
    subSkill: {
      dist: {
        ...
        intro: {
          content: (storage, player) => {
            var str = ``;
            if (player.storage.shshengxi_dist > 0)
              str += `<li>本轮内你计算与其他角色距离-${player.storage.shshengxi_dist}`;
            else if (player.storage.shshengxi_dist < 0)
              str += `<li>本轮内你计算与其他角色距离+${-1 * player.storage.shshengxi_dist}`;
            return str;
          },
        },
        sub: true,
    ...
    async content(event, trigger, player) {
      ...
      for (var current of hosts) {
        ...
        let choiceList = [
          ...
          "令" + get.translation(player) + "回复1点体力",
          "令" + get.translation(player) + "移动场上一张牌",
        ];
        ...
        const { control } = await current
          ...
          .set("choiceList", choiceList)
          .set("ai", () => {
            if (get.attitude(get.event().player, get.event().source) < 2)
              return "cancel2";
            else if (get.event().source.hp == 1 && list.includes("选项二"))
              return "选项二";
            else if (get.event().source.canMoveCard(true))
              return list[
                (Math.floor(Math.sqrt(Math.random()) * (list.length - 1)) %
                  list.length) -
                  1
              ];
            return list[0];
  ...
  shyingdang_use: {
    ...
    filter(event, player) {
      if (
        !player.storage ||
        !player.storage.shyingdang_use ||
        !player.storage.shyingdang_use.length
      )
        return false;
  ...
  shyingdang_use_clear: {
    ...
    filter: function (event, player) {
      ...
      return (
        event.player == player ||
        event.player == player.storage.shyingdang_use_source
      );
    },
    async content(event, trigger, player) {
  ...
  shfulong: {
    ...
    async content(event, trigger, player) {
      ...
      for (
        var i = 0;
        i < 3 &&
        cards.filter((card) => get.color(card, false) == "red").length > 0;
        i++
      ) {
        const { bool, links } = await player
          .chooseCardButton(
            "将任意张牌当" + get.translation(list[i]) + "使用",
            cards.filter((card) => get.color(card, false) == "red").length > 0,
            cards,
            [1, cards.length],
          )
          ...
          .set("ai", () => {
            ...
            switch (count) {
              case 0: {
                const x = game.countPlayer(function (current) {
                  return (
                    lib.filter.targetEnabled2(
                      { name: "sadouchengbingx", isCard: true },
                      player,
                      current,
                    ) &&
                    get.effect(
                      current,
                      { name: "sadouchengbingx" },
                      player,
                      player,
                    ) > 1
                  );
                ...
                if (
                  ui.selected.buttons.length < x ||
                  !player.hasValueTarget("shuiyanqijunx")
                )
                  return 1;
                return 0;
              }
              case 1: {
                const x = game.countPlayer(function (current) {
                  return (
                    lib.filter.targetEnabled2(
                      { name: "shuiyanqijunx", isCard: true },
                      player,
                      current,
                    ) &&
                    get.effect(
        ...
        cards.removeArray(links);
        await player
          .chooseUseTarget({ name: list[i], isCard: true }, links, true)
          .set("oncard", () => {
  ...
  shyishen: {
    ...
    filter: function (event, player) {
      return event.toShow?.some((i) =>
        get.character(i).skills?.includes("shyishen"),
      );
    },
    async cost(event, trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          get.prompt("shyishen"),
          "将" + get.translation(Card) + "置于一名角色的装备区",
          (card, player, target) => {
  ...
  shtujiang: {
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        const InRangers = game.filterPlayer((current) =>
          player.inRange(current),
        );
  ...
  shyingtian: {
    ...
    filter(event, player) {
      ...
      return (
        game.roundNumber >
        game.countPlayer((current) => current.group == player.group)
      );
    },
    limited: true,
    skillAnimation: true,
    ...
    async content(event, trigger, player) {
      ...
      const giv = [];
      if (!_status._shyingtian[player.playerid])
        _status._shyingtian[player.playerid] = [];
  ...
  shxingju: {
    ...
    async cost(event, trigger, player) {
      ...
      const players = game.filterPlayer(
        (current) => current.group != player.group,
      );
      const {
        bool,
        targets,
  ...
  shdianzhen: {
    ...
    async content(event, trigger, player) {
      ...
      var cards1 = cards.filter((card) => {
        return (
          player.hasUseTarget(card, true, false) && get.type2(card) == "trick"
        );
      });
  ...
  shhuitian: {
    ...
    filter(event, player) {
      if (!player.storage?._shyingtian || !player.storage._shyingtian.length)
        return false;
      var ds = Array.from(ui.discardPile.childNodes).filter((i) => {
  ...
  shjieyuan: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        ...
        .set("ai", (card) => {
          if (
            get.attitude(_status.event.player, _status.event.source) < 3 &&
            !_status.event.source.awakenedSkills.includes("shqiangfu")
          ) {
            return 7 - get.value(card);
          }
          return 0;
      ...
      if (!result.bool) {
        const resultx = await player
          ...
          .set("ai", () => {
            return (
              get.value(get.event().target.getCards("h")) >
              get.value(get.event().player.getCards("h")) + 5
            );
          })
          .set("target", target)
          .forResult();
  ...
  shqiangfu: {
    ...
    filter: function (event, player) {
      return game.hasPlayer(function (current) {
        ...
        return (
          ...
          player.canUse({ name: "sha", cards: cards }, current, false)
        );
      });
    },
    async cost(event, trigger, player) {
      event.result = await player
        .chooseTarget(
          get.prompt("shqiangfu"),
          "将一名男性角色装备区所有牌当伤害基数为转化牌数的【杀】对其使用。",
          (card, player, target) => {
    ...
    async content(event, trigger, player) {
      ...
      await player
        .useCard({ name: "sha" }, cards, target, false)
        .set("oncard", function (card) {
          const evt = get.event();
  ...
  shjizhan: {
        ...
        .set("ai", function (card) {
          if (
            ...
            get
              .centralDisCards()
              .filter(
                (card) => get.name(card) == "sha" || get.name(card) == "shan",
              ).length < 3
          )
            return 100 - get.value(card);
          return 5.5 - get.value(card);
    ...
    ai: {
      basic: {
        order: function () {
          ...
          else if (player.storage.shjizhan_ainum < 5)
            return Math.max(
              0,
              Math.max(get.order({ name: "sha" }) - 0.5, 1) -
                player.storage.shjizhan_ainum,
            );
          return 0;
        },
    ...
    subSkill: {
      sha: {
        ...
        async content(event, trigger, player) {
          await player
            ...
            .set("oncard", function (card) {
              if (get.player().storage.shxiongqi ?? false)
      ...
      damage: {
        ...
        filter(event, player) {
          return (
            event.card && event.card.storage && event.card.storage.shjizhan
          );
        },
        async content(event, trigger, player) {
          var cs = get
    ...
    filter: function (event, player) {
      ...
      if (event.filterCard({ name: name, isCard: true }, player, event))
        return player.countCards("h", (card) => {
          ...
          if (List[0] == get.name(card, false))
            return !get.is.shownCard(card) && player.storage.shyidan_blocker[0];
          return (
            List.includes(get.name(card, false)) &&
            player.storage.shyidan_blocker[List.indexOf(get.name(card, false))]
          );
        });
      return false;
    },
  ...
  shguao: {
    ...
    animationColor: "fire",
    check: function (event, player) {
      if (player.hp < 2 && !player.countCards("hs", { name: "tao" }))
        return false;
      return game.hasPlayer((current) => {
    ...
    subSkill: {
      ...
      damagesource: {
        ...
        filter: function (event, player, name) {
          return (
            player
              .getHistory("sourceDamage", (evt) => evt.num > 0)
              .indexOf(event) == 0
          );
        },
        async content(event, trigger, player) {
          game.playAudio(
            "..",
            "extension",
            "水泊娘山/audio/skill/shguzhai4.mp3",
          );
          await player.draw();
          var card = get.discardPile(function (i) {
            return i.name == "sha";
      ...
      damage: {
        ...
        filter: function (event, player, name) {
          return (
            player.getHistory("damage", (evt) => evt.num > 0).indexOf(event) ==
            0
          );
        },
        async content(event, trigger, player) {
          game.playAudio(
            "..",
            "extension",
            "水泊娘山/audio/skill/shguzhai5.mp3",
          );
          await player.draw();
          var card = get.discardPile(function (i) {
            return i.name == "shan";
  ...
  shguzhaix: {
    ...
    filterCard: function (card) {
      ...
      return (
        ["sha", "shan"].remove(get.name(ui.selected.cards[0]))[0] ==
        get.name(card)
      );
    },
    selectCard: 2,
  ...
  shjiaojiao: {
    ...
    async content(event, trigger, player) {
      ...
      var cards = player
        .getCards("h")
        .filter((card) => !get.is.shownCard(card));
      await player.discard(cards);
  ...
  shyinzhen: {
    ...
    filterTarget(card, player, target) {
      ...
      const vscard = target.getCards("he", (card) =>
        lib.skill.shdiaoman.isVisable(player, target, card),
      );
        ...
        if (evt.name == "chooseToUse") {
          game.broadcastAll(
            function (result, name) {
              ...
              lib.skill.shyinzhen_backup.prompt =
                "选择" +
                get.translation(name) +
                "（" +
          ...
          target
            .when("respond")
            .filter(
              (evt) => evt.skill == "shweizhan" && evt.card?.storage?.shweizhan,
            )
            .then(() => {
    ...
    ai: {
      ...
      skillTagFilter(player, tag) {
        return game.hasPlayer(function (current) {
          return current.hasCard(
            (card) => lib.skill.shdiaoman.isVisable(player, current, card),
            "he",
          );
        });
      },
      order: function () {
        return (
          Math.max(get.order({ name: "sha" }), get.order({ name: "shan" })) +
          0.2
        );
      },
      result: {
    ...
    subSkill: {
      ...
      draw: {
        ...
        filter(event, player) {
          return (
            event.card?.storage &&
            event.card?.storage.shyinzhen &&
            event.card?.storage.shyinzhen.isIn()
          );
        },
        content() {
  ...
  shfangyin: {
    ...
    filterCard(card) {
      ...
      if (!ui.selected.cards.length) {
        if (
          !player.hasUseTarget(
            get.autoViewAs({ name: name, nature: nature, isCard: true }),
            true,
            true,
          )
        )
          return false;
        return (
          (get.type(name) == "trick" && get.is.shownCard(card)) ||
          (get.type(name) == "basic" && !get.is.shownCard(card))
        );
      }
    ...
    filterOk() {
      ...
      return (
        ...
        cards.filter((card) => {
          return (
            (get.type(card) == "trick" && get.is.shownCard(card)) ||
            (get.type(card) == "basic" && !get.is.shownCard(card))
          );
        }).length
      );
    },
      ...
      if (get.is.shownCard(cards[0])) {
        ...
        for (var i = 0; i < cards.length; i++) {
          ...
          if (
            get.type(name) == "trick" &&
            player.hasUseTarget(
              get.autoViewAs({ name: name, isCard: true }),
              true,
              true,
            )
          )
      ...
      content(storage, player, skill) {
        ...
        const sortedList = storage
          ...
          .map((suitItem) =>
            suitItem
              .slice()
              .sort((s1, s2) => suitSortMap[s1] - suitSortMap[s2]),
          );
        if (!sortedList.length) {
          str += "无";
        } else {
          for (const suitGroup of sortedList) {
            const coloredGroup = suitGroup
              .map((suit) => getColoredSuit(suit))
              .join("、");
  ...
  shwanmeng: {
    ...
    async cost(event, trigger, player) {
      const prompt =
        get.prompt2(event.skill) +
        " <font color=#FFD700>(若仅选择自己则自己摸两张)</font>";
      event.result = await player
        ...
        .set("selectTarget", function () {
          if (
            ...
            ui.selected.targets?.includes(get.player())
          )
            return [1, 2];
          return 2;
        })
    ...
    async content(event, trigger, player) {
      const targets =
        event.targets[0].getSeatNum() <= player.getSeatNum()
          ? event.targets
      ...
      if (targets[0].getSeatNum() < player.getSeatNum()) {
        const list = ["选项二"];
        let choiceList = [
          "令" + get.translation(targets.at(-1)) + "回复1点体力",
          "令" + get.translation(targets.at(-1)) + "失去1点体力",
        ];
  ...
  shyouyun: {
    ...
    chooseButton: {
      ...
      filter(button, player) {
        ...
        return _status.event
          .getParent()
          .filterCard(card, player, _status.event.getParent());
      },
      check: function (button) {
        var player = _status.event.player;
        var card = { name: button.link[2], isCard: true };
        ...
        return {
          ...
          async precontent(event, trigger, player) {
            ...
            if (result?.card) {
              ...
              player.storage.shyouyun_round[0] =
                (result.card.viewAs || result.card.name) == "xumou_jsrg"
                  ? 1
                  : get.cardNameLength(result.card);
              player.storage.shyouyun_round[1] = game.countPlayer((current) => {
                return player.canUse(
                  { name: event.result.card.name, isCard: true },
                  current,
                  true,
                  true,
                );
  ...
  shyiqing: {
    ...
    async content(event, trigger, player) {
      ...
      const reserve = [...player.getCards("h")].sort((a, b) => {
        const getVal = (c) =>
          player.hasValueTarget(c, true, true) > 0
            ? 10 * player.getUseValue(c, true, true)
            : get.value(c);
  ...
  shwuyao: {
    ...
    async content(event, trigger, player) {
      ...
      const target = (
        await player
          .chooseTarget(
            ...
            function (card, player, target) {
              if (!target.canUse(juedou, player) && !target.canCompare(player))
                return false;
              return target != player;
            },
            true,
      ...
      let choiceList = [
        "与" + get.translation(player) + "拼点",
        "视为对" + get.translation(player) + "使用一张【决斗】",
      ];
      ...
      const { control } = await target
        ...
        .set("ai", function () {
          ...
          const hasSmall = player
            .getCards("h")
            .some((c) => [1, 2].includes(get.number(c)));
          const centerShan = get
            .centralDisCards()
            .filter((c) => get.name(c, false) == "shan").length;
          const hasBig = player.getCards("h").some((c) => get.number(c) >= 10);
          const normalize = (r) => Math.max(0, Math.min(1, r));
          const [isSelfLowHp, isSelfLowCard] = [
            selfHp <= LOW_HP,
            selfCard <= LOW_CARD,
          ];
          const [gsMore, gsLess] = [
          ...
          const [selfFewShan, selfMoreShan, selfMoreSha] = [
            selfShan <= SHAN_FEW,
            selfShan >= SHAN_MANY,
            selfSha >= SHA_MANY,
          ];
  ...
  shweizhan: {
    ...
    filterCard(card, player) {
      return (
        card.hasGaintag("shweizhan_tag1") || card.hasGaintag("shweizhan_tag2")
      );
    },
    position: "hes",
    viewAs: {
    ...
    viewAsFilter(player) {
      if (
        !player.countCards("hes", (card) => {
          return (
            card.hasGaintag("shweizhan_tag1") ||
            card.hasGaintag("shweizhan_tag2")
          );
        })
      )
        return false;
            ...
            function (cards) {
              cards.forEach((card) => {
                if (
                  ...
                  card.addGaintag("shweizhan_tag1");
                else if (
                  card.hasGaintag("shweizhan_tag1") &&
                  !card.hasGaintag("shweizhan_tag2")
                ) {
  ...
  shjuguan: {
    ...
    filter: function (event, player) {
      if (
        (player.getStat("skill").shjuguan || 0) >=
      ...
      event.result = await player
        ...
        .set("ai", (button) => {
          ...
          if (get.position(card) == "j")
            eff += get.attitude(player, target) * 2;
  ...
  shquanfu: {
    ...
    filter(event, player) {
      ...
      return event
        ...
        ?.some((i) => {
          return (
            (get.name(i, false) === "sha" && get.color(i, false) === "black") ||
            (get.subtype(i, false) === "equip1" &&
              get.name(i, false) !== "zheji") ||
            (get.subtype(i, false) === "equip2" &&
              get.name(i, false) !== "nvzhuang")
          );
        });
    },
    async content(event, trigger, player) {
        ...
        if (get.name(i, false) === "sha" && get.color(i, false) === "black") {
          var card = game.createCard2("du", i.suit, i.number),
            index = (() => {
              ...
              return max >= 0
                ? Array.from({ length: max + 1 }, (_, i) => i).randomGet()
                : null;
            })();
  ...
  shhaimou: {
    ...
    enable: "phaseUse",
    filter(event, player) {
      const List = ["jiedao", "guohe", "wuzhong"].removeArray(
        player.getStorage("shhaimou_used") || [],
      );
      for (let name of List) {
        if (
          event.filterCard({ name: name }, player, event) &&
          player.countCards("hes", { color: "black" })
        )
          return true;
      }
      return false;
    ...
    chooseButton: {
      dialog(event, player) {
        return ui.create.dialog("骇谋", [
          ["jiedao", "guohe", "wuzhong"].removeArray(
            player.getStorage("shhaimou_used") || [],
          ),
          "vcard",
        ]);
      ...
      backup(links, player) {
        return {
          ...
          check(card) {
            ...
            if (
              !player.getStorage("shhaimou_used").includes("wuzhong") &&
              checkCombo(name, player)
            )
              val -= 5;
            return 6 - val;
          },
    ...
    async content(event, trigger, player) {
      ...
      player
        ...
        .step(async (evt, trigger, player) => {
          if (
            player.storage.shxuanhe_show &&
            player.storage.shxuanhe_show.length
          ) {
            const result = await player
              .chooseBool(
                "是否获得" +
                  get.translation(player.storage.shxuanhe_show) +
                  "，令喧和视为未发动过？",
              )
              .set("ai", () => true)
              .forResult();
  ...
  shyufen: {
    ...
    async content(event, trigger, player) {
      ...
      for (const target of targets.filter(
        (current) => current.isIn() && current.countCards("he"),
      )) {
        const cards = (
          await target
            .chooseCard(
    ...
    subSkill: {
      show: {
        getcard(event, player) {
          ...
          if (
            !evt ||
            !evt.showResult?.cards.length ||
            !evt?.card?.storage?.shyufen
          ) {
            return [];
          }
        ...
        async content(event, trigger, player) {
          ...
          source.storage.shyufen.addArray(
            get.info("shyufen_show").getcard(trigger, player),
          );
        },
        sub: true,
      },
  ...
  shyanzi: {
    ...
    forced: true,
    filter: function (event, player) {
      if (event.name == "draw" || event.getParent().name == "draw")
        return false;
      const hs = player.getCards("h"),
        cards = event
          .getg(player)
          .filter((i) => hs.includes(i) && get.color(i) == "red");
      return cards.length;
    },
    async content(event, trigger, player) {
      const hs = player.getCards("h"),
  ...
  shyanzi_global: {
    ...
    enable: "chooseToUse",
    filter(event, player) {
      var skillSources = game.filterPlayer(
        (current) =>
          current != player &&
          current.hasSkill("shyanzi") &&
          current.countCards("h", (card) => card.hasGaintag("visible_shyanzi")),
      );
      for (var source of skillSources) {
        var cards = source.getCards("h", (card) =>
          card.hasGaintag("visible_shyanzi"),
        );
        for (var i of cards) {
          var card = get.autoViewAs(get.autoViewAs({ name: "tao" }, [i]));
          if (
            event.filterCard(card, player, event) &&
            (!player.hasSkill("shyanzi_global_refused") ||
              !player?.storage?.shyanzi_global_refused?.includes(source) ||
              source.getRoundHistory("damage").length > 0)
          )
            return true;
        }
      }
    ...
    subSkill: {
      refused: {
        charlotte: true,
        init: function (player) {
          if (!player.storage.shyanzi_global_refused)
  ...
  shjuluan: {
    ...
    async contentBefore(event, trigger, player) {
      ...
      const List = ["选项一"];
      let choiceList = [
        "对" + get.translation(event.targets) + "各造成1点伤害",
        "弃置" + get.translation(event.targets) + "共三张牌",
      ];
      if (
        event.targets.reduce(
          (sum, item) => sum + item.countDiscardableCards(player, "he"),
          0,
        ) >= 3
      )
        List.push("选项二");
    ...
    async content(event, trigger, player) {
      ...
      if (evt.control) {
        if (evt.control == "选项一") {
          ...
          var min = Math.max(
            3 -
              event.targets[event.targets.length - 1].countDiscardableCards(
                player,
                "he",
              ),
            0,
  ...
  shgemang: {
    ...
    filter(event, player) {
      return (
        event.getd()?.filterInD("od")?.length > 1 && _status.currentPhase.isIn()
      );
    },
    zhuSkill: true,
    //frequent: true,
    async cost(event, trigger, player) {
      event.result = await player
        .chooseButton([
          "割莽：选择获得一张牌",
          trigger.getd()?.filterInD("od").slice(0),
        ])
        .set("ai", function (button) {
          return get.value(button.link, _status.event.player);
        })
  ...
  shshuaza: {
    ...
    filter(event, player) {
      return event.filterCard(
        get.autoViewAs({ name: "wuzhong", isCard: true }),
        player,
        event,
      );
    },
    async content(event, trigger, player) {
      ...
      for (let target of targets) {
        ...
        let choiceList = [
          ...
          "视为对" + get.translation(player) + "使用【过河拆桥】",
        ];
        ...
        if (result?.control == "选项二")
          await target.useCard({ name: "guohe", isCard: true }, player);
      }
    },
  },
  shshouzi: {
  ...
  shduqiu: {
    ...
    filter(event, player) {
      if (
        ["name", "name1", "name2"].every(
          (key) => player[key] !== "sh_zhangshun",
        )
      )
        return false;
      if (
        player.name === "sh_zhangshun" &&
        !player.classList?.contains("reification")
      )
        return true;
      return (
        player?.name2 === "sh_zhangshun" &&
        !player.classList?.contains("reification2")
      );
    },
    async content(event, trigger, player) {
    ...
    set(player) {
      ...
      card.setBackgroundImage("extension/水泊娘山/image/card/sh_zhangshun.png");
      if (player.name === "sh_zhangshun") {
        ...
        if (!player.storage.shyueyong) {
          ...
          ui.cardPile.insertBefore(
            card,
            ui.cardPile.childNodes[ui.cardPile.childElementCount - 1],
          );
        }
    ...
    subSkill: {
      use: {
        ...
        filter(event, player) {
          if (
            !player.storage.shduqiu ||
            get.itemtype(player.storage.shduqiu) != "card" ||
            ui.cardPile.hasChildNodes() == false
          )
            return false;
        ...
        async content(event, trigger, player) {
          ...
          if (!player.storage.shyueyong) {
            if (
              ui.cardPile.childNodes[i] == ui.cardPile.lastChild ||
              i >= ui.cardPile.childElementCount - 1
            )
              return;
            ui.cardPile.insertBefore(
              ui.cardPile.childNodes[i + 1],
              ui.cardPile.childNodes[i],
            );
          } else {
            if (ui.cardPile.childNodes[i] == ui.cardPile.firstChild || i <= 0)
              return;
            ui.cardPile.insertBefore(
              ui.cardPile.childNodes[i],
              ui.cardPile.childNodes[i - 1],
          ...
          var current =
            history[history.length - 1].card == trigger.card
              ? history.length - 2
              : history.length - 1;
  ...
  shfuyuan: {
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        const target = result.targets[0];
        await target.damage("thunder");
        if (!_status.currentPhase || target != _status.currentPhase)
          await player.loseHp();
      }
    },
    ai: {
    ...
    intro: {
      content(storage, player) {
        ...
        if (i < ui.cardPile.childElementCount)
          return `你的武将牌在牌堆第${get.cnNumber(i + 1, true)}张`;
        return "你的武将牌未在牌堆";
      },
      markcount(storage, player) {
        var i = 0;
          ...
          effect: {
            target(card, player, target) {
              ...
              if (
                ...
                !["basic", "trick", "equip"].includes(get.type(card, "trick"))
              )
                return;
              if (zhangshun && player != target) {
                if (
                  !ui.selected.targets.length &&
                  !["nanman", "wanjian", "wugu", "taoyuan"].includes(card.name)
                )
                  return "zeroplayertarget";
                if (
                  game.countPlayer() == 2 &&
                  ["nanman", "wanjian"].includes(card.name)
                )
                  return "zeroplayertarget";
  ...
  shyueyong: {
    ...
    filter(event, player) {
      ...
      return (
        player.hasSkill("shduqiu") &&
        !player.storage.shyueyong &&
        (player.hp == 1 || zhu?.hp == 1)
      );
    },
    async content(event, trigger, player) {
      player.awakenSkill(event.name);
      ...
      player
        ...
        .step(async () => delete player.storage.shyueyong);
      player.addSkill("shyueyong2");
      player
        .when({ source: "dieAfter" })
        .step(async () => player.removeSkill("shyueyong2"));
    },
    ai: {
      combo: "shduqiu",
    },
  ...
  shsihu: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          if (
            player.countCards("h", (card) => {
              if (get.name(card) != "sha" || !player.inRange(target))
                return false;
          ...
          [
            {
              prompt:
                "请选择令" +
                get.translation(event.targets[0]) +
                "摸一到三张牌，然后其等量轮内受到【杀】的伤害+1。",
              min: 1,
              max: 3,
            },
          ],
    ...
    updateMark(player) {
      ...
      player.storage.shsihu2 = player.storage.shsihu2.filter(
        (r) => r >= game.roundNumber,
      );
  ...
  shsihu2: {
    ...
    async content(event, trigger, player) {
      var num = player.storage.shsihu2.filter(
        (r) => r >= game.roundNumber,
      ).length;
      trigger.num += num;
    },
    ...
    async content(event, trigger, player) {
      ...
      player.checkHistory("useCard", function (evt) {
        evt.targets.forEach((current) => {
          targetCount[current.playerid] =
            (targetCount[current.playerid] || 0) + 1;
        });
      });
      ...
      for (const target of targets.sortBySeat()) {
        ...
        if (player.group == "liang") {
          const list = ["出杀"];
          let choiceList = [
            "令" + get.translation(target) + "视为使用一张无距离限制的【杀】",
  ...
  sh_look2: {
    ...
    intro: {
      mark: function (dialog, content, player) {
        ...
        for (
          var i = ui.cardPile.childNodes.length - 1;
          i > ui.cardPile.childNodes.length - 7;
          i--
        ) {
          list.push(ui.cardPile.childNodes[i]);
        }
